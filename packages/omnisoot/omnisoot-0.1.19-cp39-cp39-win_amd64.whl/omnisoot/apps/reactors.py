...
class ConstantVolumeReactor(ClosedReactorMixin, ReactorAbstract, CConstUVSoot):
    ...
    def start(self):
        if self.check_soot_array(self.initial_soot):
            self.build_arrays();
            self.set_reactor_gas_mass(self.soot_gas.rho * self.reactor_volume);
            self.set_reactor_gas_volume(self.reactor_volume);
            y0 = np.hstack((self.soot_gas.rho * self.reactor_volume, self.soot_gas.T, self.soot_gas.Y_array, self.initial_soot));
            self.set_solver(t0=0, y0=y0);
...
class PressureReactor(ClosedReactorMixin, ReactorAbstract, CCVPSoot):
    ...
    def start(self):
        if self.check_soot_array(self.initial_soot):
            ...
            y0 = np.hstack((self.gas_mass, 0.0, self.soot_gas.T, self.soot_gas.Y_array, self.initial_soot));
            self.set_solver(t0=0, y0=y0);
