...
class ImageSolver(Component):
    ...
    def process(
        ...
        solution.mean_exposure_time = exposure.mean_exposure_time
        solution.exposure_duration = exposure.duration
        solution.num_quad_matches = len(registration.quad_match_list)
        solution.num_quad_matches_selected = len(quad_matches)
        solution.registration_time = registration_time
        solution.matching_time = matching_time
        solution.spotlist_uuid = spotlist.uuid
        solution.image_uuid = exposure.uuid
        try:
            solution.camera_chip_temperature = (
                exposure.chip_temperature.value + 273.15
            ) * u.K
        except AttributeError:
            self.log.warning("no chip temperature available from exposure")
        solution.mean_background = spotlist.mean_background

        return solution, match_list, stars, registration
