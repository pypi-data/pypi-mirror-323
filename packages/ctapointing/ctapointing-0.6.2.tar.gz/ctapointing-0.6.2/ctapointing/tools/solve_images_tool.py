...
from ctapointing.camera import DistortionCorrectionSIP
from ctapointing.exposure import Exposure
from ctapointing.imagesolver import ImageSolver, SkyFitter, LEDFitter, SpotType
    ...
    def start(self):
        with ExitStack() as stack:
            ...
            for spotlist in tqdm(
                ...
                exposure.ambient_pressure = 1020 * u.hPa
                exposure.ambient_temperature = 0.0 * u.deg_C
                ...
                if self.estimated_pointing_alt is not None:
                    ...
                    self.log.info(
                        f"applying AltAz estimated pointing {estimated_pointing_altaz}"
                    )
                    estimated_pointing = estimated_pointing_altaz.transform_to(ICRS)
                else:
                ...
                if len(spotlist_sky) > 0:
                    solution_reg, match_list, stars, registration = self.solver.process(
                        spotlist_sky,
                        exposure,
                        estimated_pointing,
                    )
                    ...
                    if len(match_list) != 0:
                        ...
                        solution_fit = solution_fit[0]

                        # copy stuff from the registration process
                        solution_fit.mean_exposure_time = (
                            solution_reg.mean_exposure_time
                        )
                        solution_fit.exposure_duration = solution_reg.exposure_duration
                        solution_fit.num_quad_matches = solution_reg.num_quad_matches
                        solution_fit.num_quad_matches_selected = (
                            solution_reg.num_quad_matches_selected
                        )
                        solution_fit.registration_time = solution_reg.registration_time
                        solution_fit.matching_time = solution_reg.matching_time
                        solution_fit.spotlist_uuid = spotlist.uuid
                        solution_fit.image_uuid = solution_reg.image_uuid
                        solution_fit.camera_chip_temperature = (
                            solution_reg.camera_chip_temperature
                        )
                        solution_fit.mean_background = solution_reg.mean_background
                        solution_fit.total_time = (
                            solution_reg.registration_time
                            + solution_reg.matching_time
                            + solution_fit.fitting_time
                        )
                    else:
                ...
                if len(spotlist_led) > 0:
                    ...
                    m, coord_leds_final, coord_spots_matched = self.led_fitter.process(
                        spotlist_led, exposure, flashcam
                    )

                    # center of the science camera in the image
                    science_camera_centre_pix = exposure.transform_to_camera(
                        flashcam.centre, to_pixels=True
                    )
                    print(science_camera_centre_pix)
                    print(type(science_camera_centre_pix[0][0]))

                    solution_fit.science_camera_focal_length = (
                        flashcam.sciencecameraframe.focal_length
                    )
                    solution_fit.science_camera_rotation = (
                        flashcam.sciencecameraframe.rotation
                    )
                    solution_fit.science_camera_tilt_x = (
                        flashcam.sciencecameraframe.tilt_x
                    )
                    solution_fit.science_camera_tilt_y = (
                        flashcam.sciencecameraframe.tilt_y
                    )
                    solution_fit.science_camera_offset_x = (
                        flashcam.sciencecameraframe.offset_x
                    )
                    solution_fit.science_camera_offset_y = (
                        flashcam.sciencecameraframe.offset_y
                    )

                    solution_fit.science_camera_centre_x = science_camera_centre_pix[0][
                        0
                    ]
                    solution_fit.science_camera_centre_y = science_camera_centre_pix[0][
                        1
                    ]

                    solution_fit.leds_fitted_x = coord_leds_final[:, 0]
                    solution_fit.leds_fitted_y = coord_leds_final[:, 1]
                    solution_fit.led_spots_fitted_x = coord_spots_matched[:, 0]
                    solution_fit.led_spots_fitted_y = coord_spots_matched[:, 1]
                ...
                if len(spotlist_lid) > 0:
                    solution_fit.lid_spots_x = spotlist_lid.coord_x
                    solution_fit.lid_spots_y = spotlist_lid.coord_y
                else:
                    self.log.warn("no lid spot(s) found.")

                # write results to file and database
                if solution_fit is not None:
                    if self.hdf5_writer is not None:
                        self.hdf5_writer.write(f"image_solution_{i:06d}", solution_fit)
                    if self.mongo_writer is not None:
                        self.mongo_writer.write(
                            self.mongo_output_collection, solution_fit
                        )
                else:
                    self.log.warn(
