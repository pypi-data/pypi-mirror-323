...
class SimulateImagesTool(Tool):
    ...
    def setup(self):
        ...
        try:
            ...
            self.log.error(f"unable to create image directory {self.image_url}. Make sure that parent directory exists.")
    ...
    def start(self):
        ...
        for pointing_observation in tqdm(
            ...
            if "HDF5TableWriter" in self.exposure_writers:
                with HDF5TableWriter(self.image_url / f"ctapointing_simulation_{exposure.uuid}.h5") as writer:
                    writer.write(table_name=self.hdf5_group_name, containers=exposure.to_container())
