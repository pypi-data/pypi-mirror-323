...
class SkyFitter(Component):
    ...
    def process(
        ...
        # data structure for storage of results
        solutions = [ImageSolution() for i in range(len(exposure))]
        ...
        for i, exp in enumerate(exposure):
            ...
            solutions[i]["nominal_telescope_pointing_ra"] = estimated_pointing[i].ra
            solutions[i]["nominal_telescope_pointing_dec"] = estimated_pointing[i].dec
        ...
        while num_fits <= self.max_num_fit_iterations:
            ...
            for i, exp in enumerate(exposure):
                ...
                solutions[i].residual_mean_x = delta_mean[0]
                solutions[i].residual_mean_y = delta_mean[1]
                solutions[i].residual_rms_x = delta_rms[0]
                solutions[i].residual_rms_y = delta_rms[1]
                solutions[i].num_outliers_skyfit = num_outliers
                solutions[i].num_fitted_stars_skyfit = len(delta)
                solutions[i].stars_fitted_ra = star_coords.ra
                solutions[i].stars_fitted_dec = star_coords.dec
                solutions[i].stars_fitted_x = star_coords_pix[:, 0]
                solutions[i].stars_fitted_y = star_coords_pix[:, 1]
                solutions[i].stars_fitted_mag = star_mag
                solutions[i].star_spots_fitted_x = spot_coords_pix[:, 0]
                solutions[i].star_spots_fitted_y = spot_coords_pix[:, 1]
                solutions[i].star_spots_fitted_flux = spot_flux
                solutions[i].stars_fit_converged = m.valid
                solutions[i].stars_fit_quality = (
        ...
        for i, exp in enumerate(exposure):
            ...
            solutions[i].telescope_pointing_ra = exp.telescope_pointing.ra
            solutions[i].telescope_pointing_dec = exp.telescope_pointing.dec
            solutions[i].telescope_pointing_alt = exp.telescope_pointing_altaz.alt
            solutions[i].telescope_pointing_az = exp.telescope_pointing_altaz.az

            solutions[i].star_spots_fitted_ra = spot_coords_radec.ra
            solutions[i].star_spots_fitted_dec = spot_coords_radec.dec

            solutions[i].camera_focal_length = exp.camera.focal_length[0]
            solutions[i].camera_rotation = exp.camera.rotation
            solutions[i].camera_tilt_x = exp.camera.tilt[0]
            solutions[i].camera_tilt_y = exp.camera.tilt[1]
            solutions[i].camera_offset_x = exp.camera.offset[0]
            solutions[i].camera_offset_y = exp.camera.offset[1]
            solutions[i].num_iterations_skyfit = num_fits
            solutions[i].num_outliers_skyfit = np.count_nonzero(
                [
                    True
                    for s in spot_star_match_list[i]
            ...
            solutions[i].num_fitted_stars_skyfit = len(match_list)
            solutions[i].residual_r68 = residual_radec_r68.to(u.arcsec)
            solutions[i].residual_r95 = residual_radec_r95.to(u.arcsec)

            solutions[i].fitting_time = fitting_time
            solutions[i].when_solved = when_solved.fits

            solutions[i].mean_exposure_time = exp.mean_exposure_time
            solutions[i].exposure_duration = exp.duration
            try:
                solutions[i].camera_chip_temperature = exp.chip_temperature.to(
                    u.K, equivalencies=u.temperature()
                )
            except AttributeError:
                self.log.warning("no chip temperature information from exposure")
            try:
                solutions[i].camera_temperature = exp.camera_temperature.to(
                    u.K, equivalencies=u.temperature()
                )
            except AttributeError:
                self.log.warning("no camera temperature information from exposure")
            try:
                solutions[i].camera_humidity = exp.camera_humidity
            except AttributeError:
                self.log.warning("no camera humidity information from exposure")

        self.log.info(f"finished sky fitting ({fitting_time:.2f}).")

        return solutions, m, m_inv
