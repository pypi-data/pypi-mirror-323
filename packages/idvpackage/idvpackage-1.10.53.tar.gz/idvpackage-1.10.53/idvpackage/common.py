...
def load_and_process_image_deepface(image_base64):
        ...
        try:
            ...
            try:
                ...
                # If confidence is low, try rotating
                if confidence < 0.97:
                ...
                # If first attempt fails, try with PIL rotation
                image_pil = Image.fromarray(cv2.cvtColor(image, cv2.COLOR_BGR2RGB))
                del image
                gc.collect()

                try:
                    # Try with custom rotation
                    ...
                    try:
                        # Last attempt - 180 degree rotation
                        ...
                        logging.error('Unable to extract face from ID')
                        return [], []
            ...
            if face_objs:
                try:
                    ...
                    # Clear face objects from memory
                    del face_objs, biggest_face, facial_area
                    gc.collect()

                except Exception as e:
                    logging.error(f'Error processing face encodings: {str(e)}')
                    return [], []
