...
class IdentityVerification:
    ...
    def __init__(self, credentials_string, genai_key):
        """
    ...
    def check_for_liveness(self, similarity, video_bytes, face_match_threshold=0.59):
        ...
        try:
            from idvpackage.liveness_detection import LivenessDetector
            
            # Initialize the liveness detector with both Google Vision and OpenAI
            detector = LivenessDetector(
                client=self.client,
                openai_api_key=self.genai_key
            )
            
            # Run the combined liveness check
            result = detector.check_liveness(video_bytes)

            print(f"\nFinal Liveness - Spoofing Result: {result}\n")
    ...
    def generate_facial_report_portal(self, data, latest_portal_video):
        ...
        try:
            ...
            if not initial_selfie_str:
                logging.warning("Initial selfie not available in facial report portal generation")
