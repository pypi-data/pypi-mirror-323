...
use crate::pyspark::singletons::{CodeTransformer, CodeTransformerType};
...
// trait DataframeTransformer: CodeTransformer + Debug + PartialEq + Clone + Hash {}
// impl<T: CodeTransformer + Debug + PartialEq + Clone + Hash> DataframeTransformer for T {}
            ...
            vec![PyDict(
                renames
                    ...
                    .map(|(old, new)| {
                        (
                            old.to_string(),
                            column_like!(py_lit(new.to_string())).into(),
                        )
                    })
                    .collect(),
            )
