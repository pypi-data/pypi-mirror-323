...
class ILController_RSST_AG:
    ...
    def __compute_error(self):
        if self.frequency > self.cutfrequency:
            ...
            self.B_tol = np.max(self.B_ref,axis=1)*1.1
            self.FF_tol = 1.3
    ...
    def __is_stopping_criteria_fullfilled(self):
        ...
        if np.all(S<self.tol) and np.all(FF<self.FF_tol) and np.all(B_amp<self.B_tol):
    ...
    def __store_ilc_values(self):
        ...
        self.__FFx.append(self.FF[0])
        self.__FFy.append(self.FF[1])
        self.__Sx.append(self.S[0])
        self.__Sy.append(self.S[1])
        self.__errx.append(self.err[0,:])
        self.__erry.append(self.err[1,:])
        self.__Bx_amp.append(self.B_amp[0])
        self.__By_amp.append(self.B_amp[1])
    ...
    def __write_ilc_values_to_store_setup(self):
        ...
        self.__write_ilc_value_to_store_setup("FFx",self.__FFx)
        self.__write_ilc_value_to_store_setup("FFy",self.__FFy)
        self.__write_ilc_value_to_store_setup("Sx",self.__Sx)
        self.__write_ilc_value_to_store_setup("Sy",self.__Sy)
        self.__write_ilc_value_to_store_setup("errx",self.__errx)
        self.__write_ilc_value_to_store_setup("erry",self.__erry)
        self.__write_ilc_value_to_store_setup("Bx_amp",self.__Bx_amp)
        self.__write_ilc_value_to_store_setup("By_amp",self.__By_amp)
