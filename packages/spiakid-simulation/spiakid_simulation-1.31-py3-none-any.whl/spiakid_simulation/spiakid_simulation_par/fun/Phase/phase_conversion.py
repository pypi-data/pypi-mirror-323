...
def PhaseNoise(photon, scale, seed, baseline = None):
    ...
    if len(photon[0]) > 0:
        for k in range(len(photon[0])):
            if baseline == None:
                sig[0].append(rng.normal(loc = 0,scale = scale, size = len(photon[0][k] )) + photon[0][k])
            else:
                sig[0].append(rng.normal(loc = baseline,scale = scale, size = len(photon[0][k] )) + photon[0][k])
...
def PhotonPhase(photondetect, pxnbr, conversion, nphase, decay, exptime, baseline, baselinepix, nreadoutscale,seed, k, l):
    phase = []
    if len(photondetect) > 0:
        ...
        nexpphase = PhaseExpNoise(expphase, baseline, baselinepix, nreadoutscale, seed)
...
def PhaseExpNoise(expphase, baseline, baselinepix, nreadoutscale, seed):
                
    if baseline == 'uniform':
        nexpphase = PhaseNoise(expphase, nreadoutscale, seed)
    elif baseline == 'random':
        nexpphase = PhaseNoise(expphase, nreadoutscale,seed, baselinepix)
    return(nexpphase)