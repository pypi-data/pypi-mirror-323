...
class MessageSubProtocolSet(MessageSubProtocol):
    ...
    def _subprotocol_body(self) -> bytes:
        ...
        # _LOGGER.error(f"Got a sub-protocal body with a target temp of {target_temperature}")
...
class MessageGeneralSet(MessageACBase):
    ...
    def _body(self) -> bytearray:
        ...
        # Fixes temp value of 63
        target_temperature = self.target_temperature
        # _LOGGER.error(f"Sent BEFORE adjustment {self.target_temperature}")
        if (target_temperature <= 17.0):
            target_temperature += .5
        # _LOGGER.error(f"Sent AFTER adjustment {target_temperature}")
        target_temperature = (int(target_temperature) & 0xf) | \
                             (0x10 if int(round(target_temperature * 2)) % 2 != 0 else 0)
        # _LOGGER.error(f"Sent AFTER serialization {target_temperature}")
        # Byte 3, fan_speed
...
class XA0MessageBody(MessageBody):
    """AC A0 message body."""
    #_LOGGER.error(f"Got an XA0 Message")
    def __init__(self, body: bytearray) -> None:
        ...
        # _LOGGER.error(f"Got an XA0 Message with a target temp of {self.target_temperature}")
...
class XA1MessageBody(MessageBody):
    """AC A1 message body."""
    #_LOGGER.error(f"Got an XA1 Message")
    def __init__(self, body: bytearray) -> None:
...
class XBXMessageBody(NewProtocolMessageBody):
    """AC BX message body."""
    #_LOGGER.error(f"Got an XBX Message, which is for newer addon features")
...
class XC0MessageBody(MessageBody):
    ...
    def __init__(self, body: bytearray) -> None:
        ...
        #_LOGGER.error(f"Got an XC0 Message, like the kind for Ben's office")
        super().__init__(body)
        self.power = (body[1] & 0x1) > 0
        self.mode = (body[2] & 0xE0) >> 5
        self.target_temperature = (body[2] & 0x0F) + 16.0 + (0.5 if body[0x02] & 0x10 > 0 else 0.0)
        # Fixes to get the messages recived from the units for 63 and 62F to be interpreted correctly.
        # _LOGGER.error(f"Got an XC0 message. Heard a temp of {self.target_temperature}, full byte was {body[2]}, string is {body}")
        if (self.target_temperature <= 17.5):
            self.target_temperature -= .5
            #_LOGGER.error(f"Overriding recieved temp to {self.target_temperature}")
            if (body[13] == 0x04):
            # Fix for temps of 60 and 61, since byte 2 gets reused if temp < 62, we can check
            # byte 13 to know if it's "out of range".
                self.target_temperature -= 1.0
                #_LOGGER.error(f"Temp detected below 62. Overriding recieved temp to {self.target_temperature}")
        # _LOGGER.error(f"After processing, it's now {self.target_temperature}")
        self.fan_speed = body[3] & 0x7F
...
class XC1MessageBody(MessageBody):
    ...
    def __init__(self, body: bytearray, analysis_method: int = 3) -> None:
        ...
        #_LOGGER.error(f"Got an XC1 Message, only does power consumption, which is broken")
        super().__init__(body)
...
class XBBMessageBody(MessageBody):
    """AC BB message body."""
    # _LOGGER.error(f"Got an XBB Message. NOT COMMON.")
    # These messages don't really seem to be used.
    def __init__(self, body: bytearray) -> None:
...
class MessageACResponse(MessageResponse):
    ...
    def __init__(self, message: bytearray, power_analysis_method: int = 3) -> None:
        ...
        # Seems these notify messages are recived after changing the temp (or something else?)
        # on the new wall units. Have seen them other times on ocasion. Going to just try and disable them.
