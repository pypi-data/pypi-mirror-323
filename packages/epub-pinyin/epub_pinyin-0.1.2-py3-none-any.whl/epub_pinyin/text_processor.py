...
<head>
    ...
    rt {{
        font-size: 0.85em;  /* Increased from 0.6em to 0.85em */
        font-style: normal;
        line-height: 1.2;  /* Added line height for better spacing */
        text-align: center;
        font-family: Arial, sans-serif;  /* Added a clear font for better readability */
    }}
    ruby > rb {{
        font-size: 1em;  /* Keep Chinese characters at original size */
    }}
    </style>
</head>
<body>
...
def annotate_text_with_pinyin(text: str) -> str:
    ...
    for char in text:
        if is_chinese_char(char):
            ...
            result.append(f"<ruby><rb>{char}</rb><rt>{pinyin_text}</rt></ruby>")
        else:
            result.append(char)
    return "".join(result)
