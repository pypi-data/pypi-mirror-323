    ...
    def _send_overpass_query(self, query: str) -> dict:
        ...
        for attempt in range(self.max_retries):
            try:
                ...
                # If we had a timeout but eventually succeeded, log it
                if had_timeout:
                    self.logger.info("Query completed successfully after previous timeout(s)")
                ...
                if attempt < self.max_retries - 1:
                    ...
                    self.logger.warning(f"Overpass API query timed out after {self.timeout}s, retrying in {delay} seconds...")
                    time.sleep(delay)
                else:
    ...
    def search(self, lat: Union[str, float, None] = None, 
        ...
        try:
            for tag in search_tags:
                ...
                if '~' in tag:
                    # Regex match (e.g., name~Superfund)
                    tag_key, tag_value = tag.split('~', 1)
                    # Escape quotes and special regex characters
                    tag_value = tag_value.replace('"', '\\"')
                    query = f"""
                    [out:json][timeout:{self.timeout}];
                    (
                      node["{tag_key}"~".*{tag_value}.*",i]{area_filter};
                      way["{tag_key}"~".*{tag_value}.*",i]{area_filter};
                    );
                    out body;
                    >;
                    out skel qt;
