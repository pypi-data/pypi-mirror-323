...
from ydata.preprocessors.regular.preprocessor import SynthpopPreprocessor, SynthpopSegmentPreprocessor
...
class TimeSeriesSynthesizer(BaseModel):
    ...
    def _select_pipeline_prototype(
        ...
        preprocessor = SynthpopSegmentPreprocessor
        ...
        pipeline_def = PipelinePrototype(
            dataset=PipelinePrototype.DatasetScope(
                preprocessor=SynthpopPreprocessor,
                preprocessor_params={
            ...
            block=PipelinePrototype.BlockScope(
                synthesizer=TimeSeriesSynthesizerModel.SYNTHPOP.value
            ),
        )
        return pipeline_def
    ...
    def fit(
        ...
        assert isinstance(
            X, Dataset), "X must be an instance of type Dataset. Please provide a valid input."
        assert metadata.dataset_attrs is not None, "Missing 'dataset_attrs' on Metadata instance"
        assert len(
            metadata.dataset_attrs.sortbykey), "Missing 'sortbykey' on Metadata 'dataset_attrs' attribute"

        if any([col in [w.column for w in metadata.warnings["constant"]] for col in metadata.dataset_attrs.sortbykey]):
            raise ValueError("The 'sortbykey' column cannot be constant.")
