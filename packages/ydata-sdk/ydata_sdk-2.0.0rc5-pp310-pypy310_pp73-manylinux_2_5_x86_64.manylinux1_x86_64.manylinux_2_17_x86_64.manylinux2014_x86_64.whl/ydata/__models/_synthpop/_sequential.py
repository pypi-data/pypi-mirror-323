"""Synthpop dataflow code definition Synthpop was originally developed for R:

https://cran.r-project.org/web/packages/synthpop/index.html This is YData's
python version of the R pacakage based on https://github.com/hazy/synthpop.
"""
...
from ydata.__models._synthpop._common import (intialize_smoothing_strategy, validate_datatypes,
                                              visitor_seq_to_predictor_mat)
from ydata.__models._synthpop._preprocess import SeqPreprocessor
from ydata.__models._synthpop.maps import (ENABLED_DATATYPES, METHOD_TO_TYPE_TO_FUNCTION, METHODS_MAP, SYNTHPOP_FLAVOR,
                                           Smoothing)
...
methods = METHODS_MAP[SYNTHPOP_FLAVOR.SEQ]
methods_map = METHOD_TO_TYPE_TO_FUNCTION[SYNTHPOP_FLAVOR.SEQ]


# #@typechecked
class SeqSynthpop(BaseSynthesizer):
    __name__ = "SeqSynthpop"
    DEVICE = Device.CPU
    __INDEX: str = "__index"
    ...
    def __init__(
        ...
        """Sequential Synthpop initialization.
    ...
    def fit(self, X: pdDataFrame, metadata: Metadata, dtypes: dict[str, Column] | None = None, extracted_cols: list[str] | None = None, bootstrapping_cols: list[str] | None = None) -> SeqSynthpop:
        """Fit the synthpop synthesizer models to the provided training data.
        ...
        Returns:
            SeqSynthpop: Synthesizer instance
        """
        if dtypes is None:
            warnings.warn(
        ...
        X = X.sort_values(by=metadata.dataset_attrs.entities +
                          metadata.dataset_attrs.sortbykey).reset_index(drop=True).copy()
    ...
    def sample(self, n_samples: int = 100, bootstrapping: pdDataFrame | None = None, random_state: RandomSeed = None) -> pdDataFrame:
        ...
        synth_df.drop(columns=SeqSynthpop.__INDEX,
    ...
    def __set_visit_sequence(self, bootstrapping_cols: list[str] | None = None):
        ...
        _visit_sequence = self.sortbykey_col.copy()  # [SeqSynthpop.__INDEX]
        if self.conditioning_col is not None:
    ...
    def __validate_datatypes(X, metadata, dtypes):
        validate_datatypes(ENABLED_DATATYPES[SYNTHPOP_FLAVOR.SEQ], dtypes)
        '''
