...
from ydata.utils.exceptions import DataTypeRequestError, InvalidDatasetTypeError, InvalidEntityColumnError
...
class Metadata:
    ...
    def __init__(
        ...
        self.__set_dataset_type(dataset_type)
    ...
    def __validate_dataset_attrs(self, dataset_attrs):
        try:
            if "sortbykey" in dataset_attrs and isinstance(
                dataset_attrs["sortbykey"], str
            ):
                dataset_attrs["sortbykey"] = [dataset_attrs["sortbykey"]]
            if "entities" in dataset_attrs and isinstance(
                dataset_attrs["entities"], str
            ):
                dataset_attrs["entities"] = [
                    dataset_attrs["entities"]]
            self._dataset_attrs = DatasetAttr(**dataset_attrs)
    ...
    def __validate_dataset_type(self, dataset_type, dataset_attrs):
        if dataset_type == DatasetType.TIMESERIES:
            ...
            assert "sortbykey" in asdict(
                dataset_attrs
            ), "Timeseries dataset requires sortbykey attribute."

        elif not dataset_attrs.empty():  # FIXME this should fail with dataset_attrs is a dict
    ...
    def _init_metadata(
        ...
        logger.debug("[METADATA] - Defining the dataset type.")
        self.set_dataset_type(dataset_type, dataset_attrs)
    ...
    def __create_partition(
        ...
        meta._init_metadata(
            dataset[cols], None, partition_columns, self.dataset_type)
        return meta
    ...
    def __call__(
        ...
        if len(dataset.columns) > self._partition_size:
            ...
            for i in range(n_partitions):
                ...
                if i == 0:
                    self._init_metadata(
                        dataset[cols], None, partition_columns, dataset_type)
                else:
                    meta = self.__create_partition(
                        dataset, cols, partition_columns)
                    self.combine(meta)

            # validate dataset attrs
            if isinstance(dataset_attrs, DatasetAttr) and not dataset_attrs.empty():
                self.set_dataset_attrs(**asdict(dataset_attrs))
            elif isinstance(dataset_attrs, dict) and len(dataset_attrs) > 0:
                self.set_dataset_attrs(
                    sortby=dataset_attrs.get("sortbykey"),
                    entities=dataset_attrs.get("entities")
                )
        else:
            self._init_metadata(dataset, dataset_attrs, columns, dataset_type)
        return self
    ...
    def set_dataset_type(
        ...
        """
        self.__set_dataset_type(dataset_type)
        self.dataset_attrs = dataset_attrs if dataset_attrs is not None else {}
        self.__validate_dataset_type(dataset_type, self.dataset_attrs)

    def __validate_dataset_attr_columns(
        self,
        sortby: str | List[str],
        entities: str | List[str] | None = None
    ):
        sortby = sortby if isinstance(sortby, list) else [sortby]
        if entities:
            entities = entities if isinstance(entities, list) else [entities]
        else:
            entities = []

        invalid_columns = [col for col in sortby +
                           entities if col not in self.columns]
        if invalid_columns:
            raise KeyError(
                f"The column(s) {invalid_columns} are not present on metadata")
    ...
    def set_dataset_attrs(
        ...
        """
        self.__validate_dataset_attr_columns(sortby, entities)
        dataset_attrs = {
            "sortbykey": sortby,
        }
        if entities:
        ...
        self.__validate_dataset_type(self.dataset_type, self.dataset_attrs)
    ...
    def _graph_base(self, dataset: Dataset) -> tuple[dict, dict]:
        ...
        word_count = {col: length.count("\s+").add(1).mean()
    ...
    def dataset_attrs(self, attrs: dict):
        ...
        self.__validate_dataset_attrs(attrs)
