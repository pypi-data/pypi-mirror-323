...
def validate_calculated_features(data: MultiDataset, calculated_features: list[CalculatedFeature]) -> None:
    ...
    def validate_feature(cf: CalculatedFeature, feat: str, field: str, check_column: bool = True):
        ...
        if table_name not in tables:
            raise_error(
                f"{cf} {field} [{feat}] invalid table name [{table_name}]")
    ...
    for cf in calculated_features:
        ...
        if len(feature_tables) != 1:
            raise_error(
                f"Multiple features calculation is only allowed for the features of the same table [{cf}]")
        source_tables = get_tables_from_columns(cf.calculated_from)
        # TODO this is probaby never true anymore, since we calculate from all parents
        if len(source_tables) != 1:
            raise_error(
                f"All source data must pertain to the same table [{cf}]")
