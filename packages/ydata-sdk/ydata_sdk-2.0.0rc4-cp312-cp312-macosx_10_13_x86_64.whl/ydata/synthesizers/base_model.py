...
# replace by other pickle save and load methods
...
from ydata.synthesizers.exceptions import (MissingDeviceException, NotFittedException, SegmentationStrategyException,
                                           TemporaryPathException)
...
class BaseModel:
    ...
    def _init_fit(
        ...
        self.entities = metadata.dataset_attrs.entities
    ...
    def _fit(self, X: Dataset, metadata: Metadata, n_entities: int = None,
        ...
        for col in constant_cols:
            if col not in metadata.dataset_attrs.entities:
...
generated, please fit the synthesizer first."
    ...
    @log_time_factory(logger)
    def _check_invalid_values(self, metadata: Metadata, holdout: float):
        if metadata.shape[0] <= BaseModel.MIN_ROWS and not self._bypass_min_rows_limit:
            raise NotEnoughRows(
