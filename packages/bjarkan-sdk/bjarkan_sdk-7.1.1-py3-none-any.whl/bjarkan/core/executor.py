    ...
    def _prepare_margin_params(self, exchange_id: str, order: OrderConfig, base_params: Dict) -> Dict:
        ...
        if order.margin_mode:
            if exchange_id == 'binance':
                ...
                # Bybit spot margin configuration
                params['isLeverage'] = 1  # Must be 1 for spot margin trading
                params['leverageType'] = 'cross_margin'  # Specify cross margin
                # Optional: auto-borrow settings if needed
    ...
    async def execute_order(self, order: OrderConfig) -> Dict:
        ...
        for exchange_id, amount, price in execution_plan:
            ...
            try:
                ...
                base_params = {'timeInForce': order.time_in_force} if order.type == 'limit' else {}

                # Add margin-specific parameters
                params = self._prepare_margin_params(exchange_id, order, base_params)

                # Log the execution attempt
                logger.info(f"Executing order on {exchange_id}: {order.side} {amount} {self.symbol} "
                            f"at {price if order.type == 'limit' else 'market'} "
