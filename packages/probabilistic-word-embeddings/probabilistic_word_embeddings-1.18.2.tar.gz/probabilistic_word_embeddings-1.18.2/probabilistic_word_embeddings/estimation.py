...
def map_sgd(embedding, data_generator=None, N=None, lr=0.01, model="sgns", epochs=5, loglevel="DEBUG"):
    prior_ratio = N / len(embedding.vocabulary)
    ...
    for epoch in range(epochs):
        for _ in progressbar.progressbar(range(N)):
            ...
            eta = 1.0 / (1.0 + np.exp(-np.dot(rho, alpha)))
            minus_eta = 1.0 - eta
            if x == 1.0:
                embedding[i] += alpha * minus_eta * lr
                embedding[j] += rho * minus_eta * lr
            elif x == 0.0:
                embedding[i] += -alpha * eta * lr
                embedding[j] += -rho * eta * lr

            prior_wd = random.choice(list(embedding.vocabulary))
            embedding[prior_wd] += - embedding[prior_wd] * embedding.lambda0 * lr
    return embedding
