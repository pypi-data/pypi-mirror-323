...
def process_proportion_stats(df, metrics_config, results_list=None):
    """
    Calculate proportion statistics for different metrics and add results to results list.
    ...
    metrics_config : dict
        Dictionary containing metric configurations with column names
        Example format:
        {
            'trial_conversion': {
    ...
    results_list : list, optional
        List to store results dictionaries. If None, creates new list.
    
    Returns:
    --------
    list
        Updated results list with new statistics
    """
    ...
    for metric_name, metric_config in metrics_config.items():
        ...
        for index, row in df[(df['variant'] != 'control') & (df['variant'] != 'holdout')].iterrows():
            ...
            value = row[success_col] / row[total_col]
