...
#specific heat capcity in J/( kg K )
c_Si3N4 = 3.51 * 1e3
def SiN(Ekin): #enter kinetic energy in eV
    x = Ekin+E_mu #Total energy
    beta = np.sqrt(1-(E_mu**2/x**2))    #Lorentz beta
    gamma = 1/np.sqrt(1-beta**2)        #Lorentz gamma
    D_SiN = np.log((4*E_e**2*beta**4*gamma**4)/((1+2*gamma*(E_e/E_mu)+(E_e/E_mu)**2)*I_Si3N4**2))
    y_SiN = K/(beta**2)*j_Si3N4*(0.5*D_SiN-beta**2) * rho_Si3N4 #Energy loss [MeV/cm]
