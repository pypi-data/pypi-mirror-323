...
def take_last(s: pd.Series):
    return s.iloc[-1]


def take_first(s: pd.Series):
    return s.iloc[0]
...
def bin_variable(s, min_value, max_value, width, how,
    ...
    :param how: function to use for binning. Use np.ceil, np.floor
...
def __bin_series(s: pd.Series, min_value, max_value, width, how,
                 default, lower, upper, errors):
    s_binned = s.copy()
    nan_values = np.array(s_binned.isna()) | np.array((s_binned == errors))

    conditions = np.array([True for _ in range(len(s_binned))])
    ...
    s_binned.loc[conditions] = list(
        np.clip(
            a=(how((s_binned.loc[conditions] - min_value) / width)
               * width) + min_value,
            a_min=min_value,
            a_max=max_value)
    )
    s_binned = np.round(s_binned, 8)
...
def __bin_value(s: numeric_types, min_value, max_value, width, how,
    ...
    if (math.isnan(s) or s == errors) and default is not None:
        ...
        if not assigned:
            result = np.clip(
                a=how((s - min_value) / width) * width + min_value,
                a_min=min_value,
                a_max=max_value
            )
...
def multiple_replace(s: pd.Series, replacing_dict: dict, default=None):
    """
...
def fill_empty_with_distr(s: pd.Series, values_to_fill: list,
...
def fill_str(s: pd.Series, length, fill_value: str = '0', fillna=False):
