"""Command-line interface for ReasonChain."""
...
try:
    ...
    raise ImportError(
        "click is required for ReasonChain CLI. "
...
try:
    ...
    raise ImportError(
        "rich is required for ReasonChain CLI. "
...
@click.option('--max-tokens', default=750, help='Maximum tokens per response')
@click.option('--temperature', default=0.2, help='Temperature for response generation')
@click.option('--timeout', default=30.0, help='Timeout in seconds for API requests')
@click.option('--debug', is_flag=True, help='Enable debug output')
def reason(query: str, model: Optional[str], max_tokens: int, temperature: float,
          timeout: float, debug: bool) -> int:
...
def ui(port: int) -> None:
    ...
    try:
        console.print(f"[green]Starting ReasonChain UI on port {port}...[/green]")
