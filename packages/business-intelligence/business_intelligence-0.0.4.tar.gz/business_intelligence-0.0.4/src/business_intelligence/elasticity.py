...
class config_table:
    ...
    def calculate_elasticity(self, cod_art=None, q_range=5):
        ...
        self.data['range_price_avg'] = (self.data[self.price_col] / self.data[self.quantity_col]).round(0)

        # remove the rows with zero price or quantity
        self.data = self.data[(self.data['range_price_avg'] > 0) & (self.data[self.quantity_col] > 0)]

        # Calculate the total 
        prices_unique = self.data['range_price_avg'].sort_values(ascending=True).unique()
        demand = []
        for price in prices_unique:
            demand.append(self.data[self.data['range_price_avg'] == price][self.quantity_col].sum())
        ...
        table['var_range_price_avg'] = table['range_price_avg'].pct_change()
