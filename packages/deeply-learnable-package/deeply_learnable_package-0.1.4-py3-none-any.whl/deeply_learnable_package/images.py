...
def chars_random():
    ...
    with zipfile.ZipFile('chars.zip', 'r') as zip_ref:
        zip_ref.extractall('chars')

    class AugmentedCharDataset(Dataset):
        def __init__(self, root_dir, base_transform, aug_transform, num_augments=5):
            self.root_dir = root_dir
            ...
            # Сбор всех оригинальных и аугментированных изображений
            for cls in self.classes:
                ...
                for img_name in os.listdir(cls_dir):
                    ...
                    self.samples.append((self.base_transform(image), self.class_to_idx[cls]))
                    
                    # Генерация аугментированных версий
                    for _ in range(self.num_augments):
                        aug_image = self.aug_transform(image)
                        self.samples.append((aug_image, self.class_to_idx[cls]))
    ...
    def main():
        ...
        def train_model(use_augmentation=False):
            if use_augmentation:
                dataset = AugmentedCharDataset(
                    root_dir='chars/chars',
                    ...
                    def __init__(self, root_dir, transform):
                        ...
                        for cls in self.classes:
                            ...
                            for img_name in os.listdir(cls_dir):
                                img_path = os.path.join(cls_dir, img_name)
                                image = Image.open(img_path).convert('RGB')
                                self.samples.append((self.transform(image), self.class_to_idx[cls]))
                ...
                dataset = SimpleDataset('chars/chars', base_transform)
            ...
            return f1, train_losses, test_losses

        # Обучение моделей
        print('Обучение без аугментаций:')
        f1_base, train_loss_base, test_loss_base = train_model(False)

        print('\nОбучение с аугментациями:')
        f1_aug, train_loss_aug, test_loss_aug = train_model(True)
        ...
        plt.plot(test_loss_base, label='Test Loss')
        plt.title('Без аугментаций')
        plt.legend()
        
        plt.subplot(1,2,2)
        plt.plot(train_loss_aug, label='Train Loss')
        plt.plot(test_loss_aug, label='Test Loss')
        plt.title('С аугментациями')
        plt.legend()
        
        plt.show()

        print(f'\nРезультаты:\nБез аугментаций F1: {f1_base:.4f}\nС аугментациями F1: {f1_aug:.4f}')
