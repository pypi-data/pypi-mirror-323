...
def seconds2Datetime(linuxTimeSeconds:int):
    from datetime import datetime
    dt = datetime.fromtimestamp(linuxTimeSeconds)
...
apsItems = header0['Index']# PVs, logged in the apstrim files
listOfApsTimestamps = list(header0['Directory'].keys())
apsTimeRange = listOfApsTimestamps[0],listOfApsTimestamps[-1]
print(f'Start,Stop logging time: {[seconds2Datetime(i) for i in apsTimeRange]}')

for i,item in enumerate(apsItems):
    cell = item.rsplit(':',1)[1]
    lcolor = hsvToRgb(i, len(apsItems))
