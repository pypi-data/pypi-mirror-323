...
class XParticle(iGeo, Particle):
    ...
    # lambda methods
    # --------------------------------------------------------
    def _pre_average(self, key, old_value, new_value, keys):
        """
        ## case dict
    ...
    def _pre_max_old(self, key, old_value, new_value, edge):
        if old_value is None:
            old_value = {
                "max": {},
            }
        ...
        if (cache := self._lambdas_cache):
            for key in cache.intersection(new_value):
                ...
                if (current := old_value.get(key)) is None:
                    ...
                    old_value[key] = op(current, value)
        else:
    ...
    def _pre_max(self, old_value, new_value, keys):
        return self._pre_lambda(old_value, new_value, max)


    def _pre_min(self, old_value, new_value, keys):
        return self._pre_lambda(old_value, new_value, min)


    def _pre_sum(self, old_value, new_value, keys):
        return self._pre_lambda(old_value, new_value, operator.add)


    def _pre_freq(self, old_value: Dict, new_value, keys):
        """
    ...
    def _post_freq(self, key, values, keys):
        """
    ...
    def _get_wip_completed_edge(self) -> EDGE:
        ...
        while self._wip_stream:
            ...
            if idt < t1:
                ...
                edge = {
                    MONOTONIC_KEY: wdata.wget(MONOTONIC_KEY),
    ...
    async def _wip_build_walk(self, data) -> List:
        ...
        for lambda_mapper in self._lambda_mapper:
            mapping_keys = lambda_mapper.mapping_keys(
                {}, only_defined=False
            )
        ...
        async for data in super()._compute(edge, ekeys):
            ...
            for key in self.available_lambdas.intersection(data):
                ...
                if isinstance(sub_data, list):
                    ...
                    # 2nd mappers
                    for lambda_mapper in self._lambda_mapper:
                        sub_data = lambda_mapper.remap(sub_data, only_defined=False)
