...
from DocTest.CompareImage_legacy import CompareImage
...
class VisualTest(object):
    ...
    def validate_images(self, reference_image, test_image):
        if not os.path.isfile(reference_image) and not is_url(reference_image):
            raise AssertionError(f'Reference file {reference_image} does not exist.')
        if not os.path.isfile(test_image) and not is_url(test_image):
            raise AssertionError(f'Test file {test_image} does not exist.')


    def create_reference_image(self, reference_image, test_image):
    ...
    def compare_images(self, reference_image: str, test_image: str, placeholder_file: str=None, mask: Union[str, dict, list]=None, check_text_content: bool=False, move_tolerance: int=None, contains_barcodes: bool=False, get_pdf_content: bool=False, force_ocr: bool=False, DPI: int=None, watermark_file: str=None, ignore_watermarks: bool=None, ocr_engine: str=None, resize_candidate: bool=False, blur: bool=False , threshold: float =None ,**kwargs):
        ...
        if self.reference_run is True:
            self.create_reference_image(reference_image, test_image)
            return
        self.validate_images(reference_image, test_image)
