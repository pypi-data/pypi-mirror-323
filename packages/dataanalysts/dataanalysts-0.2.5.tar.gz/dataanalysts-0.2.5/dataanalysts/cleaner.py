...
def clean(df, strategy=None, **kwargs):
    ...
    try:
        if strategy == 'remove_duplicates':
            ...
            strategy_type = kwargs.get('strategy', 'mean')
            value = kwargs.get('value', None)
            if strategy_type == 'fill':
                df.fillna(value, inplace=True)
                print(f"Filled missing values with specified values: {value}.")
            elif strategy_type in ['mean', 'median', 'mode']:
                for col in df.select_dtypes(include=['number']).columns:
                    if strategy_type == 'mean':
                        df[col].fillna(df[col].mean(), inplace=True)
                    elif strategy_type == 'median':
                        df[col].fillna(df[col].median(), inplace=True)
                    elif strategy_type == 'mode' and not df[col].mode().empty:
                        df[col].fillna(df[col].mode()[0], inplace=True)
                print(f"Filled missing values using {strategy_type} strategy.")

        elif strategy == 'fix_structural':
            column = kwargs.get('column')
            if column in df.columns:
                strategy_type = kwargs.get('strategy', 'lowercase')
                if strategy_type == 'lowercase':
                    df[column] = df[column].str.lower()
                elif strategy_type == 'uppercase':
                    df[column] = df[column].str.upper()
                print(f"Fixed structural issues in column {column} using {strategy_type} strategy.")
    ...
    try:
        while True:
            ...
            if option == '1':
                ...
                value = eval(input("Enter value for 'fill' strategy (as a dictionary): ") or "{}").strip()
                df = clean(df, strategy='handle_missing', strategy_type=strategy, value=value)
            elif option == '2':
                df = clean(df, strategy='remove_duplicates')
            elif option == '3':
                column = input("Enter column to fix structural errors: ").strip()
                strategy = input("Enter fix strategy (lowercase/uppercase): ").strip()
                df = clean(df, strategy='fix_structural', column=column, strategy=strategy)
            elif option == '4':
