...
def generate_docs(file_path_or_src: str | object, is_package: bool = False, depth=2) -> str:
    """Generate markdown documentation for a given Python file."""
    if not isinstance(file_path_or_src, str):
        import inspect
        import pydoc
        from types import ModuleType, MethodType, FunctionType, FrameType, CodeType
        ...
        if isinstance(file_path_or_src, (ModuleType, type, MethodType,Callable, FunctionType,FrameType,CodeType)):
            try:
                ...
                try:
                    file_path_or_src = getattr(file_path_or_src, '__file__', getattr(file_path_or_src, '__module__').__file__)  
                except:
...
def main(path: str, output_file: str=None, *,use_regex=False,stream=False):
