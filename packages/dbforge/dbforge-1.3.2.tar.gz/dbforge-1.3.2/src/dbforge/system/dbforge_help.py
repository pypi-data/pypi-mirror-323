...
def get_version():
    ...
    try:
        # Start from the current file's directory and go up to the project root
        current_dir = Path(__file__).resolve()
        root_dir = current_dir.parent.parent.parent.parent  # Navigate up to the root directory
        pyproject_path = root_dir / "pyproject.toml"
        
        if not pyproject_path.exists():
            # Fallback: try looking in the current working directory
        ...
        print(f"Error reading version: {e}")  # Debug line - you can remove this later
        return "unknown"
