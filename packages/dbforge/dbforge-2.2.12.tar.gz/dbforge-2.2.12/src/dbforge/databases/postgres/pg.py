...
class PostgresManager:
    ...
    def ensure_connection(self) -> None:
        ...
        # First try direct connection without Docker
        try:
        ...
        # If localhost and direct connection failed, try Docker setup
        if (self.host == "localhost" or self.host == "127.0.0.1"):
            ...
            try:
                containers = client.containers.list(
                    filters={"ancestor": "postgres", "status": "running"}
                )
                
                # Try to connect to running containers
                for container in containers:
                    try:
                        ports = container.ports
                        for container_port in ports:
                            if "5432/tcp" in container_port:
                                host_port = int(ports[container_port][0]["HostPort"])
                                self.port = host_port
