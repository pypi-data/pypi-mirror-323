...
from ...system.dbforge_docker import ensure_docker
...
class PostgresManager:
    def __init__(self, host: str = "localhost", port: int = 5432, 
        ...
        # Always start with the provided host
        self.host = host
        self.port = port
        self.user = user
        ...
        # Ensure Docker is available if using localhost
        if self.host == "localhost" or self.host == "127.0.0.1":
            if not ensure_docker():
                raise Exception("Docker is required for local PostgreSQL management")
    ...
    def ensure_connection(self) -> None:
        ...
        # First check if we have any running postgres containers
        result = subprocess.run(
            ["docker", "ps", "--filter", "ancestor=postgres", "--format", "{{.Ports}}"],
            capture_output=True,
            text=True
        )
        
        # Parse port mappings from running containers
        for port_mapping in result.stdout.splitlines():
            if "0.0.0.0:" in port_mapping:
                try:
                    # Extract port from format like "0.0.0.0:5432->5432/tcp"
        ...
        # If we get here and host is localhost, try to set up a new instance
        if self.host == "localhost" or self.host == "127.0.0.1":
        ...
        # If we get here, we couldn't connect to any postgres
