...
def install_docker_ubuntu():
    ...
    try:
        ...
        subprocess.run(["curl", "-fsSL", "https://download.docker.com/linux/ubuntu/gpg", "|", "sudo", "gpg", "--dearmor", "-o", "/etc/apt/keyrings/docker.gpg"], check=True)
        subprocess.run(["sudo", "chmod", "a+r", "/etc/apt/keyrings/docker.gpg"], check=True)
        
        # Add the repository to Apt sources
        repo_cmd = 'echo "deb [arch="$(dpkg --print-architecture)" signed-by=/etc/apt/keyrings/docker.gpg] https://download.docker.com/linux/ubuntu "$(. /etc/os-release && echo "$VERSION_CODENAME")" stable" | sudo tee /etc/apt/sources.list.d/docker.list > /dev/null'
