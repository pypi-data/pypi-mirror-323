...
class SkewWeightedNormalization(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X):
        ...
        # Ensure input dimensions match fitted dimensions
        if X.shape[1] != self.mu_.shape[0]:
            raise ValueError(
                f"Shape mismatch: Input data has {X.shape[1]} features, but the transformer was fitted "
                f"with {self.mu_.shape[0]} features."
            )
    ...
    def inverse_transform(self, X):
        """
        Reverse the skew-weighted normalization for the transformed data.

        Parameters:
        ----------
        X : array-like of shape (n_samples, n_features)
            Transformed data to revert back to the original scale.
        ...
        """
        X = np.asarray(X)
        original_data = (X - self.beta * np.tanh((X * self.k * self.sigma_) / self.mu_)) \
                        * (self.sigma_ * (1 + self.alpha * np.abs(self.gamma_))) + self.mu_
