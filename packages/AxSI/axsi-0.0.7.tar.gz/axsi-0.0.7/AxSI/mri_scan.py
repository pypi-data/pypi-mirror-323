...
def save_nifti(fname: str, img: np.ndarray, affine: np.ndarray) -> None:
    """
...
def build_file_name(subj_folder: Path, file_name: str) -> Path:
    """
    Build path to save files in a specific manner
    """
    return Path(os.path.join(subj_folder, 'AxSI', file_name + '.nii.gz'))
