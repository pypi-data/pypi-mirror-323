...
class Reservoir:
    ...
    def __init__(self, size: int, observation_interval_in_ms: int, object: Dict = None):
        ...
        if not object:
            ...
            self._queue_end_time = datetime.now().timestamp()
            self._first_queue_index = 0
            self._intermediary_record = None
            self.statistics = {
                'count': 0,
                'sum': 0,
                'max': None,
            ...
            self._queue_end_time = object['_queue_end_time']
            self._first_queue_index = object['_first_queue_index']
            self._intermediary_record = object['_intermediary_record']
    ...
    def to_plain_object(self):
        ...
        return {
            ...
            '_queue_end_time': self._queue_end_time,
            '_first_queue_index': self._first_queue_index,
            '_intermediary_record': self._intermediary_record,
    ...
    def _update_queue(self):
        ...
        if empty_elements_count > 0:
            ...
            self._queue_end_time += intervals_count * self._interval
    ...
    def _take_time_intervals_count(self) -> float:
        ...
        time_diff = time_now - self._queue_end_time
    ...
    def _update_running_statistics_on_remove(self, remove_count: int):
        remove_element_index = self._first_queue_index + 1
    ...
    def _add_record(self, empty_elements_count):
        if self._intermediary_record is not None:
            self.fill_array(self._first_queue_index)
            self.array[self._first_queue_index] = self._intermediary_record
            self._intermediary_record = None
        cur_index_in_array = self._update_running_statistics_on_remove(empty_elements_count)
        self._first_queue_index = cur_index_in_array
    ...
    def _update_intermediary_record(self, el):
        if self._intermediary_record is None:
            self._intermediary_record = {
                'count': 1,
                'sum': el,
                'max': el,
                'min': el,
            ...
            if self._intermediary_record['max'] < el:
                self._intermediary_record['max'] = el
            if self._intermediary_record['min'] > el:
                self._intermediary_record['min'] = el
            self._intermediary_record['count'] += 1
            self._intermediary_record['sum'] += el
            self._intermediary_record['sumOfSquares'] += math.pow(el, 2)
    ...
    def _find_min(self, index):
        ...
        for i in range(len(self.array)):
            ...
            if el is not None and el['min'] is not None and el['min'] < min and i != index:
                min = el['min']
        if min == math.inf:
            return self._intermediary_record['min'] if self._intermediary_record is not None else None
        return min
    ...
    def _find_max(self, index):
        ...
        for i in range(len(self.array)):
            ...
            if el is not None and el['max'] is not None and el['max'] > max and i != index:
                max = el['max']
        if max == -math.inf:
            return self._intermediary_record['max'] if self._intermediary_record is not None else None
        return max
