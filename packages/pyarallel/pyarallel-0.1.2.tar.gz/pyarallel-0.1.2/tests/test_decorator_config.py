...
class TestDecoratorOverrides:
    ...
    def test_decorator_overrides_global_defaults(self, config_manager):
        ...
        decorated_func = parallel(max_workers=6, executor_type="process", batch_size=20)(self.sample_function)
        
        # Verify decorator args override global defaults
        assert decorated_func.max_workers == 6  # Override from default 4
        assert decorated_func.executor_type == "process"  # Override from default "thread"
...
def test_runtime_config_warnings():
    ...
    with warnings.catch_warnings(record=True) as w:
        ...
        @parallel(executor_type="process", batch_size=1)  # Process pool with small batch size
