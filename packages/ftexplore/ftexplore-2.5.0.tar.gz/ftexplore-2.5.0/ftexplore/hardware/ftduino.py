...
class WebUSB():
    def __init__(self, vendor_id, product_id, on_data_callback=None, idle_data=None):
        ...
        # Find backend
        backend = usb.backend.libusb1.get_backend()  # only works if libusb is in path
        if backend is None:
            backend = usb.backend.libusb1.get_backend(find_library=lambda x: "C:\\Windows\\system32\\libusbk.dll")
            libusb_path = os.path.join(os.path.dirname(usb.backend.libusb1.__file__), '..', '..', 'libusb')
            if not os.path.exists(libusb_path):
                logger.error(f'libusb library not found in [{libusb_path}]; please install it via "pip install libusb"')
            logger.debug(f'libusb is installed in {libusb_path}')
            dll_path = os.path.join(libusb_path, '_platform', '_windows', 'x64', 'libusb-1.0.dll')
            if not os.path.exists(libusb_path):
                logger.error(f'libusb1.dll library not found in [{dll_path}]')
            logger.debug(f'libusb1.dll is installed in {dll_path}')
            backend = usb.backend.libusb1.get_backend(find_library=lambda x: dll_path)
        if backend is None:
            raise IOError('libusb library not found')
        logger.info(list(usb.core.find(backend=backend, find_all=True, idVendor=vendor_id, idProduct=product_id)))
        # Find USB device
        self.dev = usb.core.find(idVendor=vendor_id, idProduct=product_id)
        if self.dev is None:
            raise IOError('Hardware device not found')
...
class HardwareFtDuino(base.HardwareBase):
    ...
    def show_error(self):
        '''Show error info in GUI'''
        self.enqueue_event('on_show_messagebox_requested', type='error', title='ftDuino missing', message='ftDuino not found or not accessible. Please check if ftDuino is connected properly and not in use and then restart the application.')
    ...
    def on_quit(self, metadata):
        ...
        if self.ftduino is None:  # nothing to do if not connected
            return None
        self.set_led(False)
    ...
    def set_motor_hardware(self, num, speed, metadata={}):
        ...
        assert num <= 3
        if self.ftduino is None:
            logger.warning('Can\'t set motor state; ftDuino not available')
            return None
        motor = str(num + 1)
        if speed < 0:
            mode = 'left'
