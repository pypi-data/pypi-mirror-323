...
class QueryParam(InjectedParam):
    ...
    model_loader_fn: Callable[[Any], Any] | None = dataclasses.field(default=None)
    ...
    def get_value(self, args: Any, kwargs: dict[str, Any]):
        ...
        return _convert_value_to_type(self, values, self.target_type)
...
def param(
    ...
    *,
    ...
    model_loader_fn: Callable[[Any], Any] | None = None,
) -> T:
    return cast(
        T,
        QueryParam(
            ...
            model_loader_fn=model_loader_fn,
        ),
    )
...
def _convert_value_to_type(qp: QueryParam, values: list[Any], target_type: type):
    # List type
    if get_origin(target_type) is list:
        list_type = get_args(target_type)[0]
        return [_convert_value_to_type(qp, [v], list_type) for v in values]

    if target_type is list:
        return [_convert_value_to_type(qp, [v], str) for v in values]
    ...
    try:
        if value is None:
            ...
            if isinstance(value, model_type):
                return value
            try:
                if qp.model_loader_fn is not None:
                    return qp.model_loader_fn(value)
