...
def get_roi_map(DATA_DIR):
    ...
    id_to_acronym = (
        id_to_acronym | ccf_structures.set_index("id")["acronym"].to_dict()
    )
    acronym_to_id = {
...
def build_length_df(dataDF, graphs, DATA_DIR):
    """
    ...
    trim_df = False
    normalize_df = True
    id_to_roi, id_to_acronym, id_to_parent = get_roi_map(DATA_DIR)
    ...
    for i, graph in graphs.items():
        ...
        for region in regions:
            ...
            nodes_in_region = {
                i for i, node in nodes if node["allen_id"] == region
            }
            ...
            edges_in_region = [
                (u, v)
                for u, v in edges
                if u in nodes_in_region or v in nodes_in_region
            ]
        ...
        lengthDict[i] = {
            id_to_acronym[key]: val for key, val in roi_lengths.items()
        }
