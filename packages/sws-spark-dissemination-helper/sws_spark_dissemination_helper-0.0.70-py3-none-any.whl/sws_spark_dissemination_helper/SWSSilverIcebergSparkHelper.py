...
class SWSSilverIcebergSparkHelper:
    ...
    def remove_null_duplicates(
        ...
        df_duplicates = df.filter(col("diss_flag")).withColumn(
            "count", F.count(lit(1)).over(window_spec)
        )
        duplicate_rows = df_duplicates.collect()
        if len(duplicate_rows) > 0:
            raise RuntimeError(
                f"There are some duplicates in the data that are flagged for dissemination: {str(duplicate_rows)}"
            )
