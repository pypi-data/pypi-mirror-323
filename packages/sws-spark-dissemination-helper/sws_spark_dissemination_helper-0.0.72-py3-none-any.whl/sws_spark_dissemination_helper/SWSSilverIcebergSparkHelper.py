...
class SWSSilverIcebergSparkHelper:
    ...
    def check_duplicates(
        ...
        if df_duplicates.count() > 0:
            df.writeTo(f"{self.iceberg_tables.SILVER}_duplicates").createOrReplace()
            raise RuntimeError(
                f"There are some duplicates in the data that are flagged for dissemination they can be checked in the {self.iceberg_tables.SILVER}_duplicates table"
            )
