...
def map_codes(
    ...
    (
        df_intermediate.filter(col("is_duplicate"))
        .writeTo(
            f"AwsDataCatalog.sws_dissemination_tags_silver.aproduction_validated_{col_name.lower()}"
        )
        .createOrReplace()
    )

    df_mapping.writeTo(
        f"AwsDataCatalog.sws_dissemination_tags_silver.mapping_code_correction"
    ).createOrReplace()

    # Remove the columns that were not in the original dataset
    # return df_intermediate.drop(
    #     "new_dim_code",
    #     "delete",
    #     "multiplier",
    #     "partition_column",
