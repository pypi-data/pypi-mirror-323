...
class StationList(object):
    ...
    def _parse_shakemap_json(self, jfile, min_nresp, sta_set, imt_set, amp_set):
        ...
        if "type" not in stas:
            logging.warning("%s appears to contain no stations, skipping", jfile)
            return ([], [], [])
        if stas["type"] != "FeatureCollection":
            logging.warning("%s is not a ShakeMap JSON stationlist, skipping", jfile)
            return ([], [], [])
        ...
        for sl in root.iter("stationlist"):
            ...
            for station in sl:
                ...
                if "code" not in attributes:
                    logging.warning("Station does not have station code: skipping")
                    continue
