...
def press(key,repeat=1):
    for i in range(repeat):pydirectinput.press(key)
...
def close2Mission(TH=13,offsetY=40):
    global Xc,Yc
    while True:
        img = np.array(pyautogui.screenshot())
        try:
            y,x=findColors(img,[235, 223, 55],10)[0]
            y+=offsetY
            print(x,y)
            if x>Xc+TH:press('d')
            elif x<Xc-TH:press('a')
            elif y>Yc+TH:press('s')
            elif y<Yc-TH:press('w')
            else:print(abs(Xc-x)+abs(Yc-y));break
        except:pass
    
def close2enemy(TH=40):
    global Xc,Yc
    img = np.array(pyautogui.screenshot())
    try:
        y,x=findColors(img,[255,0,0],0)[0]
        print(x,y)
        if x>Xc+TH:press('d')
        if x<Xc-TH:press('a')
        if y>Yc+TH:press('s')
