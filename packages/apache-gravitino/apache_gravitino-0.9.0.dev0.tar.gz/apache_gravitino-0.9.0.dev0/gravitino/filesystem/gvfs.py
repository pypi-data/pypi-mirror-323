...
class GravitinoVirtualFileSystem(fsspec.AbstractFileSystem):
    ...
    storage_prefixes = {
        ...
        f"{StorageType.OCI.value}://": StorageType.OCI,
    }
    ...
    def rm(self, path, recursive=False, maxdepth=None):
        ...
        if isinstance(
            fs,
            (
                self.lazy_load_class("s3fs", "S3FileSystem"),
                self.lazy_load_class("ocifs", "OCIFileSystem"),
            ),
        ):
    ...
    def _convert_actual_path(
        ...
        elif storage_location.startswith(f"{StorageType.OCI.value}://"):
            ops = infer_storage_options(storage_location)
            if "host" not in ops or "username" not in ops or "path" not in ops:
                raise GravitinoRuntimeException(
                    f"Storage location:{storage_location} doesn't support now, the bucket_name, "
                    f"namespace_name, and path are required in the storage location."
                )
            actual_prefix = f"{ops['username']}@{ops['host']}{ops['path']}"
    ...
    def _convert_actual_info(
        ...
        elif "timeModified" in entry:
            # OCI
            time_modified = entry["timeModified"]
            if isinstance(time_modified, datetime):
                # when call fs.ls(ls_dir, detail=True), time_modified is a datetime object
                last_modified = time_modified.timestamp()
            else:
                # when call fs.info(), time_modified is a string
                last_modified = datetime.strptime(
                    time_modified, "%a, %d %b %Y %H:%M:%S GMT"
                ).timestamp()
    ...
    def _recognize_storage_type(path: str):
        ...
        """
        for prefix, storage_type in GravitinoVirtualFileSystem.storage_prefixes.items():
    ...
    def _get_filesystem(
        ...
        try:
            ...
            elif storage_type == StorageType.OCI:
                new_cache_value = self._get_oci_filesystem(
                    fileset_catalog, name_identifier
                )
            else:
                raise GravitinoRuntimeException(
    ...
    def _get_oci_filesystem(self, fileset_catalog: Catalog, identifier: NameIdentifier):
        # todo: support credential for oci

        options = self._options or {}
        oci_configs = {
            key[len(GVFSConfig.GVFS_FILESYSTEM_OCI_CONFIG_PREFIX) :]: value
            for key, value in options.items()
            if key.startswith(GVFSConfig.GVFS_FILESYSTEM_OCI_CONFIG_PREFIX)
        }

        return (
            TIME_WITHOUT_EXPIRATION,
            importlib.import_module("ocifs").OCIFileSystem(**oci_configs),
        )
