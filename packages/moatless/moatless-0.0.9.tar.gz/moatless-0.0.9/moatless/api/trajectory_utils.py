...
from moatless.api.schema import (
    TrajectoryDTO, NodeDTO, ActionDTO, ObservationDTO, CompletionDTO, 
    ActionStepDTO, FileContextDTO, UpdatedFileDTO, UsageDTO, FileContextFileDTO, FileContextSpanDTO
)
...
def convert_moatless_node_to_api_node(node: Node, action_history: Dict[str, str]) -> NodeDTO:
    ...
    if node.file_context and node.file_context.test_files:
        all_test_results = [result.model_dump() for test_file in node.file_context.test_files
    ...
    return NodeDTO(
        ...
        fileContext=file_context_to_dto(node.file_context, node.parent.file_context if node.parent else None) if node.file_context else None,
        error=node.error,
