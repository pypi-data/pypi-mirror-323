...
class TestAssociationAnalyzer(unittest.TestCase):
    ...
    def test_preprocess_text(self):
        ...
        self.assertTrue(
            "woman work hard scientist discover groundbreaking cure disease ."
            == processed[0]
        )

    def test_fit_transform(self):
        # Test TF-IDF fitting and transformation
        tfidf_matrix, feature_names, processed_docs = self.analyzer.fit_transform(
            self.documents
        )
        self.assertEqual(
            tfidf_matrix.shape[0], len(self.documents)
        )  # Rows match number of documents
    ...
    def test_normalize_tfidf_difference(self):
        ...
        normalized = self.analyzer.normalize_tfidf_difference(
            avg_tfidf, overall_avg_tfidf
        )
    ...
    def test_analyze_subgroup_terms(self):
        ...
        results = self.analyzer.analyze_subgroup_terms(
            self.documents, self.subgroup_terms
        )
        self.assertIn("woman", results)
        self.assertIn("man", results)
        self.assertTrue(
            "engineer" in [word for word, _ in results["woman"]["associated_words"]]
        )
        self.assertTrue(
            "athlete" in [word for word, _ in results["man"]["associated_words"]]
        )
    ...
    def test_get_top_associations(self):
        ...
        for (word, score), (expected_word, expected_score) in zip(
            top_associations, expected_top_4
        ):
