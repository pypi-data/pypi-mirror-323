...
def generate_random_device_info(user_agent: str) -> dict:
    ...
    real_device = {
        ...
        # 随机是否禁用 cookies
        "disable_cookies": bool(random.getrandbits(1)),
        "touch_mode": {
            "enabled": touch_enabled,
            "maxTouchPoints": (2 if touch_enabled else 0)
        },
        "screen_size": {
...
def apply_device_info(fp, device_info):
    """
    ...
    # 设置地理位置（经纬度）
    fp.set_setGeolocation(latitude=device_info['latitude'],
    ...
    fp.set_user_agent(
        ...
        acceptLanguage=device_info['accept_language']
    )

    # 设置触摸模式
    fp.set_touch_mode(
        enabled=device_info['touch_mode']['enabled'],
        maxTouchPoints=device_info['touch_mode']['maxTouchPoints']
    )
