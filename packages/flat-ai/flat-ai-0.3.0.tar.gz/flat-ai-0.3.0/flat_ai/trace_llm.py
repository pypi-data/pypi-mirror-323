...
def configure_logging(log_destination=None):
    ...
    if isinstance(log_destination, str):
        ...
        raise ValueError("Invalid log_destination. Must be a file name (str) or StringIO object.")
...
class MyOpenAI(sys.modules['openai'].OpenAI):
    ...
    def post(self, *args, **kwargs):
        ...
        logger.openai_trace(f"[{call_id}] Arguments: {json.dumps(args, indent=2, ensure_ascii=True)}")
        
        # Process kwargs to handle JSON strings recursively
        processed_kwargs = self._process_json_values(kwargs)
        logger.openai_trace(f"[{call_id}] Keyword arguments: {json.dumps(processed_kwargs, indent=2, default=str, ensure_ascii=True)}")
        
        # Call the original post method with original kwargs
        result = super().post(*args, **kwargs)
        
        # Check if this is a streaming response
        is_streaming = isinstance(kwargs.get('body'), dict) and kwargs['body'].get('stream', False)
            ...
            'choices': [{
                ...
                'message': {
                    'content': (
                        ...
                        if isinstance(choice.message.content, str) and (
                            isinstance(kwargs.get('body'), dict) and 
                            isinstance(kwargs['body'].get('response_format'), dict) and
                            kwargs['body']['response_format'].get('type') == 'json_object'
        ...
        tokens_per_second = result.usage.total_tokens / elapsed_time if elapsed_time > 0 else 0
        logger.openai_trace(f"[{call_id}] Timing metrics:")
        logger.openai_trace(f"[{call_id}] - Total time: {elapsed_time:.2f} seconds")
        logger.openai_trace(f"[{call_id}] - Tokens per second: {tokens_per_second:.2f}")
        logger.openai_trace(f"[{call_id}] - Result: {json.dumps(result_dict, indent=2, ensure_ascii=True)}")
