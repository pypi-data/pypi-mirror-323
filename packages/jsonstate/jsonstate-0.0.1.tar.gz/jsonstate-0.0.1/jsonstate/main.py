...
from jsonstate import __version__
...
def init_new_package(package_name: str) -> None:
    """Clone jsonstate files into `package_name` and run `make init` in it."""
    new_package_path = Path.cwd() / package_name

    pipe = Popen(  # noqa: S603
        ['git', 'clone', 'https://github.com/albertas/jsonstate', new_package_path],  # noqa: S607
        stdin=PIPE,
        stdout=PIPE,
    )
    pipe.communicate()[0]
...
def main() -> None:
    ...
    if parsed_args.version:
        print(f'jsonstate {__version__}')  # noqa: T201
