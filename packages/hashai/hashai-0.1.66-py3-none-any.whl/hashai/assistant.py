...
class Assistant(BaseModel):
    ...
    def print_response(
        ...
        **kwargs,
        ...
        if isinstance(image, str) and image.startswith("http"):
            # Directly pass the URL to the LLM
            return self.llm_instance.generate_from_image_url(message, image, **kwargs)

        # Handle local image (PIL.Image)
        elif isinstance(image, Image):
            try:
                # Convert the image to JPEG format and bytes
                image_bytes = io.BytesIO()
                image = image.convert("RGB")  # Ensure the image is in RGB mode
                image.save(image_bytes, format="JPEG")  # Save as JPEG
                image_bytes = image_bytes.getvalue()

                # Generate response using base64-encoded JPEG image
                return self.llm_instance.generate_from_image(message, image_bytes, **kwargs)
            except Exception as e:
        ...
        # Generate and return the response for standard text-based queries
