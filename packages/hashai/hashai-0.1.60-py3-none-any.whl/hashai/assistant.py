...
class Assistant(BaseModel):
    ...
    def _generate_response_from_image_url(self, image_url: str, **kwargs) -> str:
        """
        Send the image URL to the LLM for analysis if the LLM supports vision.
        """
        if not self.llm_instance.supports_vision:
            raise ValueError("Vision is not supported for the current model.")
        
        # Use OpenAI's chat completions API with the image URL
        response = self.llm_instance.generate_from_image_url(image_url, **kwargs)
        return response

    def _generate_response_from_image(self, image: Union[str, Image], markdown: bool = False, **kwargs) -> str:
        """
        ...
        try:
            ...
            if isinstance(image, str) and image.startswith("http"):
                ...
                return self.llm_instance.generate_from_image_url(image, **kwargs)

            # Handle local image (PIL.Image)
            elif isinstance(image, Image):
                # Convert the image to bytes
                image_bytes = io.BytesIO()
                image.save(image_bytes, format="PNG")  # Save as PNG (or any supported format)
                image_bytes = image_bytes.getvalue()

                # Generate response using base64-encoded image
                return self.llm_instance.generate_from_image(image_bytes, **kwargs)
    ...
    def print_response(
        ...
        **kwargs,
        ...
        # If the message is an image URL, process it
        if isinstance(message, str) and message.startswith("http"):
            response = self._generate_response_from_image_url(message, **kwargs)
            print(response)
            return response
    ...
    def _generate_response_from_image(self, image: Image, markdown: bool = False, **kwargs) -> str:
        """
        Send the image to the LLM for analysis if the LLM supports vision.
        ...
        try:
            # Check if the LLM supports vision
            if self.llm_instance and self.llm_instance.supports_vision:
                # Supported formats for saving the image
                supported_formats = ["PNG", "JPEG", "BMP", "TIFF"]
                response = None
            
                for fmt in supported_formats:
                    try:
                        # Convert the image to bytes in the current format
                        image_bytes = io.BytesIO()
                        image.save(image_bytes, format=fmt)
                        image_bytes = image_bytes.getvalue()

                        # Send the image to the LLM
                        response = self.llm_instance.generate_from_image(image_bytes, format=fmt, **kwargs)
                    
                        # If the LLM processes the image successfully, return the response
                        if response:
                            if markdown:
                                return f"**Response:**\n\n{response}"
                            return response
                    except Exception as format_error:
                        logger.warning(f"Failed to process image in format {fmt}: {format_error}")
            
                # If all formats fail, return an error message
                return "Failed to process the image in all supported formats."

            else:
                # Fallback to OCR if vision is not supported
                return self._process_image_with_ocr(image)
    ...
    def _process_image_with_ocr(self, image: Image) -> str:
        """
        ...
        try:
            ...
            return f"An error occurred while processing the image with OCR: {e}"
