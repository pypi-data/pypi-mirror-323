...
def write_rust_struct(
    ...
    *,
    ...
    out.write(f"struct {cls.__name__} {{\n")
    ...
    for ii, (field_name, attr_as_type) in enumerate(
        ...
        # General case
        field_rust_type = convert_type_to_rust(attr_as_type.origin)
...
def convert_type_to_rust(py_type: type) -> str:
    ...
    if as_type.origin is list:
        return f"Vec<{convert_type_to_rust(as_type.args[0].origin)}>"

    if as_type.origin is tuple:
        return (
            f"({', '.join(convert_type_to_rust(arg.origin) for arg in as_type.args)})"
        )

    if as_type.origin is set:
        return f"HashSet<{convert_type_to_rust(as_type.args[0].origin)}>"

    if as_type.origin is dict:
        return f"HashMap<{convert_type_to_rust(as_type.args[0].origin)}, {convert_type_to_rust(as_type.args[1].origin)}>"

    if as_type.origin is t.Optional:
        return f"Option<{convert_type_to_rust(as_type.args[0].origin)}>"
...
def main() -> None:
    ...
    with Path("uniserde_models.rs").open("w") as out:
        ...
        for model_cls in all_model_classes:
            write_rust_model(model_cls, out)
            out.write("\n\n")
