...
def _get_attribute_map(
    ...
    # Already cached?
    try:
        return serde._field_map_cache[value_type]
    except KeyError:
        pass
    ...
    result = {
        ...
        for (
            attribute_py_name,
            attribute_type,
        ) in utils.get_class_attributes_recursive(value_type).items()
    }

    # Cache the result
    serde._field_map_cache[value_type] = result
...
def create_lazy_instance(
    ...
    result._uniserde_field_definitions_ = _get_attribute_map(
        value_type, serdeserializer
    )
