...
class SerdeCache(ABC, t.Generic[IN, OUT]):
    ...
    _all_handler_builders: dict[t.Type[IN], HandlerBuilder]

    def __init__(
        self,
        *,
        eager_class_handler_builders: dict[t.Type[IN], HandlerBuilder],
        lazy_class_handler_builders: dict[t.Type[IN], HandlerBuilder],
        override_method_name: str,
        user_provided_handlers: dict[t.Type, Handler],
        lazy: bool,
        ...
        self._handler_cache: dict[t.Type, Handler] = user_provided_handlers.copy()

        # This is used by the lazy wrapper to cache field information
        self._field_map_cache: dict[t.Type, dict[str, tuple[str, t.Type]]] = {}
    ...
    def _get_handler(
        ...
        try:
            return self._handler_cache[type_key]
        except KeyError:
            pass
        ...
        try:
            ...
            return lambda serde, value_in, value_type: override_method(
                value_in,
                serde,
                value_type,
            )
        ...
        handler_builder = self._get_handler_builder(value_type, type_key)
        handler = self._create_handler_from_handler_builder(
            handler_builder,
            value_type,
            type_key,
        )
        ...
        # Cache it for later
        self._handler_cache[type_key] = handler
        return handler
    ...
    def _get_handler_builder(
        ...
        try:
            override_method = self._user_provided_handlers[value_type]
        except KeyError:
            pass
        else:
            return self._create_handler_builder_from_handler(override_method)
        ...
        try:
            return self._all_handler_builders[type_key]
        except KeyError:
            pass
        ...
        return type(self)._build_field_by_field_class_handler  # type: ignore
    ...
    def _create_handler_builder_from_handler(
        self,
        handler: Handler,
    ) -> HandlerBuilder:
        """
        ...
        def handler_based_handler_builder(
            ...
            gen.write(
                f"{result_var} = {handler_var}({serde_var}, {input_variable_name}, {value_type_var})",
            )
            return result_var
    ...
    def _create_handler_from_handler_builder(
        ...
        for (
            external_value_value_type,
            external_variable_name,
        ) in gen.external_processors():
            # Don't run in circles!
            external_value_type_key = utils.get_type_key(external_value_value_type)

            if external_value_type_key == type_key:
                raise NotImplementedError(
                    f"TODO: Processing `{type_key}` depends on itself. This is not yet implemented."
                )

            scope[external_variable_name] = self._get_handler(
                external_value_value_type,
                external_value_type_key,
            )
