...
def generate_word_document(
    ...
    print(f"Generating Word document with header: {header}, save_pdf: {save_pdf}")
    ...
    try:
        if save_pdf:
            ...
            if send_email:
                ...
                # Send the email
                try:
                    with smtplib.SMTP_SSL("smtp.gmail.com", 465) as server:  # SMTP server for Gmail
                        server.login(from_email, password)
                        server.send_message(msg)
                    print(f"Email sent successfully to {to_email} with the attached document: {file_name}")
                except Exception as e:
                    print(f"Failed to send email: {str(e)}")

        else:
            response = {"status": "skipped", "message": "Saving the Word document is disabled."}
