...
def straight(
    *,
    ...
    bulge_region = (bulge_margin + bulge_taper_length,
    ...
    *,
    ...
    if length is None:
        ...
        if 4 * radius >= abs_offset:
            default_length = _pf.grid_ceil((abs_offset * (4 * radius - abs_offset)) ** 0.5)
...
def rectangular_spiral(
    *,
    ...
    port_spec = _get_default(function, "port_spec", port_spec)
    turns = _get_default(function, "turns", turns)
    radius = _get_default(function, "radius", radius)
...
def route_taper(
    *,
    ...
    offset_distance: _typ.Union[float, tuple[float, float]] = None,
    use_box: bool = None,
    technology: _pf.Technology = None,
    name: str = None,
):
    ...
    use_box = _get_default(function, "use_box", use_box, 0)
    name = _get_default(function, "name", name, "")
    ...
    if layer == ():
        ...
        if terminal1.routing_layer != terminal2.routing_layer:
            _warn.warn(f"Terminals have different routing layers. Using {layer}.", RuntimeWarning)
...
def route_manhattan(
    *,
    ...
    layer: tuple[int, int] = None,
    width: float = None,
    overlap_fraction: _typ.Union[float, tuple[float, float]] = None,
