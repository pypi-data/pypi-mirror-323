...
def downscale_qf(range_max, i_scale, bitwidth=8, scale_bits=8):
    ...
    scale, s_out = to_fixed_point(o_scale, bitwidth=scale_bits, signed=False)
...
def downscale_fp(range_max, i_scale, bitwidth=8, scale_bits=8):
    ...
    scale, in_shift = to_fixed_point(1.0 / i_scale, bitwidth=scale_bits, signed=False)
...
def downshift(range_max, i_scale, bitwidth=8):
    ...
    _, out_shift = to_fixed_point(range_max, bitwidth=bitwidth, signed=True, clamp=True)
    ...
    shift = align_to(out_shift, i_scale.ndim) / i_scale
