...
from ..quantization.outputs import downscale, downshift
from ..quantization.activations import compute_lut_values
...
class QuantizedConv2D(OnnxLayer):
    ...
    def __quantize__(self, qinput, force_fp=False):
        ...
        if "LUT" in self.op_type:
            ...
            weights_dict.update({prefix + "M": scale.astype("uint8"), prefix + "S_act": s_out})
            # Compute lut values
            ...
            i_scale = align_to(i_scale, qweights.ndim)
            # Values stored in the table were quantized with a power-of-two as a scale,
            # in order to apply a downshift to the output.
            # Note output scale is not need at this point (already applied when downscaling
            # lut inputs)
            range_max = self.weights["range_max"]
            s_out, ocalib_scale = downshift(range_max, i_scale)
    ...
    def build_subgraph(op_type):
        ...
        # LUT (optional)
        shift_t_names = ["Scale", "Shift"]
        if "LUT" in op_type:
            nodes[-1].output.__setitem__(0, nodes[-1].op_type)
            # Intermedial downscale
            nodes += get_scale_out_ops(nodes[-1].output[0], "Ys",
                                       shift_name="ActShift", bitwidth=11)
            # Main operation
            nodes += get_lut_ops("Ys", "Yi")
            # Remove final scale (not required)
            shift_t_names[0] = ""
