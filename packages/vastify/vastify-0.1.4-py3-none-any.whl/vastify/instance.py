...
class Instance:
    ...
    def __init__(self, instance_id):
        ...
        self.ssh_addr = instance_state['SSH Addr']
        self.ssh_port = int(instance_state['SSH Port'])
        self.ssh_user = 'root'
    ...
    def blacklist_instance(self):
        ...
        save_pickle('blacklist.pkl', blacklist)
    ...
    @staticmethod
    async def from_requirements(requirements: dict):
        instance_id = await get_active_or_launch_instance(requirements)

        await Instance.wait_until_started(instance_id)

        return Instance(instance_id)

    @staticmethod
    async def wait_until_started(instance_id: str, timeout: int = 300):
        ...
        instance_state = get_instance_state(instance_id)
        while instance_state['Status'] != 'running':
            ...
            if asyncio.get_event_loop().time() - start_time > timeout:
                ...
                return None
        logger.info(
            f'Instance finished starting, new state: {instance_state["Status"]}')
