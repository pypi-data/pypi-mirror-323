...
class DATABASE:
    ...
    def WriteIndex(self, information, cell, index: int):
        if os.path.exists(f"{self.fileName}.{self.fileType}"):
            ...
            linesI = linesW.index(str([line for line in linesW if line.split('|')[1] == str(index)][0]))

            linesW[linesI] = f"{str(cell)}|{index}|{information_str}|{str(cell)}|{index}\n"
        ...
        if os.path.exists(f"{self.fileName}.{self.fileType}"):
            if not os.path.exists(f"{newFileName}.{self.fileType}"):
                ...
                if self.logS == 'BASE':
                    ...
                    print(Fore.YELLOW + f"Database file '{Fore.GREEN}{self.fileName}.{self.fileType}{Fore.RED}' Renaming!!! {Fore.RESET}")
                ...
                if self.logS == 'BASE':
                    ...
                    print(Fore.YELLOW + f"Database file '{Fore.GREEN}{self.fileName}.{self.fileType}{Fore.RED}' Renaming completed!!! New name >> {Fore.GREEN}{newFileName}.{self.fileType}{Fore.RESET}")
