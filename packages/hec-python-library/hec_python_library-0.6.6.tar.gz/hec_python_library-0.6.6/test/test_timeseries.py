...
def test_expand_collapse_trim() -> None:
    ...
    # ----------------------------------------#
    ts2 = ts.asTimeZone("US/Pacific")
    ts2.iselect(
        lambda tsv: tsv.time == HecTime("2024-10-20T01:00:00-07:00")
    ).isetProtected()
    ts2.iselect(lambda tsv: tsv.time == HecTime("2024-10-25T01:00:00-07:00"))
    # print(ts.data)
    expectedLength1 = len(ts)
    expectedTimes1 = ts2.times[:]
    expectedValues1 = ts2.values[:]
...
def test_merge() -> None:
    ...
    times = pd.Index(  # type: ignore
        [(start_time + i * intvl).datetime() for i in range(len(values1))], name="time"
    )
...
def test_snapToRegular() -> None:
    ...
    # print(ts2.data)
    expectedTimeVals = [
        HecTime("2024-10-15 04:10:00") + i * Interval.getCwms("4Hours")
        for i in range(6)
    ]
    ...
    # print(ts2.data)
    expectedTimeVals = [
        HecTime("2024-10-15 00:10:00") + i * Interval.getCwms("4Hours")
        for i in range(6)
    ]
    expectedTimes = [f"{t.date(-13)} {t.time(True)}" for t in expectedTimeVals]
