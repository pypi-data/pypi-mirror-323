...
class Garlic:
    ...
    def generate_response(self, query: str, chat_history: Optional[List] = None) -> Dict:
        """Generate response with main content, sponsored section, and recommendation"""
        try:
            ...
            # Format main response with sponsored content
            main_response = self.prompt_manager.format_prompt(
                query=query,
                companies=selected_companies,
                chat_history=chat_history
            )
            
            # Generate recommendation based on the main response
            recommendation = self.prompt_manager.format_recommendation_prompt(
                query=query,
            ...
            return {
                "query": query,
                "main_response": "I couldn't find any relevant companies.",
                "recommendation": "💡 Would you like to try a different search query?",
                "companies": []
            }
    ...
    def parse_response(self, response: Dict) -> Dict:
        ...
        try:
            main_text = response.get("main_response", "")
            
            # Split into main response and sponsored sections
            sections = main_text.split("[Sponsored]")
            
            main_content = sections[0].replace("[Main Response]", "").strip()
            sponsored_content = sections[1].strip() if len(sections) > 1 else ""
            ...
            return {
                "query": response.get("query"),
                "main_content": "Error parsing response.",
                "sponsored_content": "",
                "recommendation": "",
                "companies": []
            }
