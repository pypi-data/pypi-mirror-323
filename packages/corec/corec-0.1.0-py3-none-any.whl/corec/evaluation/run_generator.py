...
class RunGenerator(BaseModel):
    ...
    train_path: Optional[FilePath] = Field(
        default=None, description="Path to the training data, used during the post-filtering."
    )
    ...
    preds_item_idx: NonNegativeInt = Field(
        default=1, description="Index for the item id column in the predictions."
    )
    preds_score_idx: NonNegativeInt = Field(
        default=2, description="Index for the score column in the predictions."
    )
    preds_test_item_idx: NonNegativeInt = Field(
        default=3, description="Index for the test item id column in the predictions."
    )
    preds_sep: str = Field(
    ...
    num_processors: PositiveInt = Field(
        default=1, description="Number of processes to run in parallel."
    )
