...
from scfetch.vpn import get_vpn_status
...
def main():
    ...
    parser.add_argument("--ascii", type=str, help="specify the ascii art")
    args = parser.parse_args()

    # define the ascii art
    if args.ascii:
        lines = define_ascii(args.ascii)
    else:
        lines = define_ascii()
    # read the config
    config = read_config()
    ...
    print(f"{lines[18]}{Effect.BOLD}{color}VPN Status{Color.OFF}{Effect.OFF}:       {get_vpn_status()}")
    if args.ascii:
        if args.ascii == "linux":
