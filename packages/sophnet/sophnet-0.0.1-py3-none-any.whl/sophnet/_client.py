...
from ._exceptions import SophNetError, APIStatusError
...
__all__ = ["Timeout", "Transport", "ProxiesTypes", "RequestOptions", "SophNet", "AsyncOpenAI", "Client", "AsyncClient"]


class SophNet(SyncAPIClient):
    ...
    def __init__(
        ...
        *,
        ...
        if api_key is None:
            raise SophNetError(
                "The api_key client option must be set either by passing api_key to the client or by setting the SOPHNET_API_KEY environment variable"
            )
        ...
        if base_url is None:
            base_url = os.environ.get("SOPHNET_BASE_URL")
        if base_url is None:
            base_url = f"https://www.sophnet.com/api/open-apis"
...
class OpenAIWithRawResponse:
    def __init__(self, client: SophNet) -> None:
