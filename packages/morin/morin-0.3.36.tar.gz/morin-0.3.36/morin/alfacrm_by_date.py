...
class ALFAbyDate:
    ...
    def get_customer(self, date):
        try:
            final_result = []
            for branch in self.branches_list:
                final_result += self.get_basic(f'v2api/{str(branch).strip()}/customer/index', 'created_at_to', self.common.flip_date(date),'created_at_from', self.common.flip_date(date))
    ...
    def get_lesson(self, date):
        try:
            final_result = []
            for branch in self.branches_list:
                final_result += self.get_basic(f'v2api/{str(branch).strip()}/lesson/index', 'created_at_to', self.common.flip_date(date),'created_at_from', self.common.flip_date(date))
    ...
    def get_pay(self, date):
        try:
            final_result = []
            for branch in self.branches_list:
                final_result += self.get_basic(f'v2api/{str(branch).strip()}/pay/index', 'created_at_to', self.common.flip_date(date),'created_at_from', self.common.flip_date(date))
