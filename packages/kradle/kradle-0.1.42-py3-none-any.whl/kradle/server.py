...
class Kradle:
    ...
    def serve_agent(cls, agent_class, slug, port=None, tunnel=True, host='localhost'):
        ...
        if not instance._app:
            ...
            if slug != "*":
                ...
                # Making the agent URL and slug available to the agent class
    ...
    def create_session(cls, api_key, challenge_slug, agents):
        ...
        try:
            ...
            logger.info(f"View it live: {base_url}/workbench/sessions/{session_id}")
            ...
            for agent in agent_list:
                try:
                    ...
                    logger.error(f"Failed to shutdown agent: {shutdown_error}")
            
            if isinstance(e, requests.RequestException):
                logger.error(f"Unable to reach Kradle Workbench at {base_url}")
