...
def load_dill(folder, filename, mode="rb"):
    required_package = "dill"
    try:
        package = __import__(required_package)
    except ImportError:
...
def load_anything(data_folder, filename, mode="rb",protocol:Union["pkl", "dill"]="pkl"):
    if protocol == "pkl":
        return load_pickle(data_folder, filename, mode)
        
    elif protocol == "dill":
        return load_dill(data_folder, filename, mode)
    else:
        raise ValueError(f"Protocol: {protocol} not supported") 
