...
def get_batch_size(in0: torch.Tensor, in1: torch.Tensor, in2: torch.Tensor):
    bs_0 = in0.size(0) if in0.dim() == 2 else None
    bs_1 = in1.size(0) if in1.dim() == 2 else None
    bs_2 = in2.size(0) if in2 is not None and in2.dim() == 2 else None
    result = None
    if bs_0 is not None and bs_0 != 1:
        result = bs_0
    if bs_1 is not None and bs_1 != 1:
        torch._assert(result is None or bs_1 == result, "mismatch in batch size")
        result = bs_1
    if bs_2 is not None and bs_2 != 1:
        torch._assert(result is None or bs_2 == result, "mismatch in batch size")
        result = bs_2
    if result is None:
        result = 1
    return result
