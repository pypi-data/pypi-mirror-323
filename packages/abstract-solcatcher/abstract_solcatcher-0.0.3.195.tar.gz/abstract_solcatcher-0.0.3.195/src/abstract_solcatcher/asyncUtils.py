...
def run_in_event_loop(coro):
    """
    Execute a coroutine in the appropriate context.
    If inside an existing event loop, use `asyncio.create_task`.
...
def call_rate_limiter(method, params, url_1_only=True, url_2_only=False, **kwargs):
    return run_in_event_loop(async_call_rate_limiter(method=method, params=params, url_1_only=url_1_only, url_2_only=url_2_only, **kwargs))

def make_rate_limited_call(method, params, url_1_only=True, url_2_only=False, **kwargs):
    return run_in_event_loop(async_make_rate_limited_call(method=method, params=params, url_1_only=url_1_only, url_2_only=url_2_only, **kwargs))

def call_solcatcher_py(endpoint, *args, **kwargs):
    return run_in_event_loop(async_call_solcatcher_py(endpoint, *args, **kwargs))

def call_solcatcher_ts(endpoint, *args, **kwargs):
    return run_in_event_loop(async_call_solcatcher_ts(endpoint, *args, **kwargs))

def call_solcatcher_db(endpoint, *args, **kwargs):
    return run_in_event_loop(async_call_solcatcher_db(endpoint, *args, **kwargs))
