...
async def async_make_request(url, payload, headers=None):
    ...
    logging.debug(f"Making async POST request to {url} with payload: {payload} and headers: {headers}")
...
def make_request(url=None, payload={}, headers=None, url_1_only=None, url_2_only=False,*args, **kwargs):
    ...
    return get_async_response(async_make_request,url=url,payload=payload,headers=headers or get_headers())
