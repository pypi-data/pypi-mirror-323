...
from heaserver.service.util import now, filter_mapping
...
class _StorageMetadata:
    ...
    all_objects: dict[str, list[tuple[bool, datetime]]] = field(default_factory=dict)
...
async def _list_object_versions(s3_client: S3Client, groups: Mapping[str, _StorageMetadata], bucket_name: str):
    list_partial = partial(s3_client.get_paginator('list_object_versions').paginate, Bucket=bucket_name)
    loop = asyncio.get_event_loop()
    pages = await loop.run_in_executor(None, lambda: iter(list_partial()))
    while (obj := await loop.run_in_executor(None, next, pages, None)) is not None:
        for obj_ in obj.get('Versions', []):
            metadata = groups[obj_['StorageClass']]
            if obj_['Key'] not in metadata.all_objects:
                metadata.all_objects[obj_['Key']] = []
        ...
        for delete_obj_ in obj.get('DeleteMarkers', []):
            if delete_obj_['Key'] not in metadata.all_objects:
                metadata.all_objects[delete_obj_['Key']] = []
            ...
            metadata.first_modified = delete_obj_['LastModified'] if metadata.first_modified is None or delete_obj_['LastModified'] < metadata.first_modified else metadata.first_modified
            metadata.last_modified = delete_obj_['LastModified'] if metadata.last_modified is None or delete_obj_['LastModified'] > metadata.last_modified else metadata.last_modified
        await asyncio.sleep(0)
    ...
    for metadata in groups.values():
        for versions in metadata.all_objects.values():
            versions_ = iter(sorted(versions, key=lambda x: x[1]))
            while True:
                versions_part = list(_takewhile_inclusive(lambda x: not is_delete_marker(x), versions_))
                if versions_part:
