      ...
      for (unsigned int iSys = 0; iSys < _systems.size(); iSys++) {
        ...
        for_spin(transitionDensity, _fnameSuffix) {
          std::string filename = _systems[iSys]->getSystemPath() + _systems[iSys]->getSystemName() +
                                 "_TransitionDensity" + _fnameSuffix_spin + std::to_string(i + 1);
        ...
        if (SCFMode == UNRESTRICTED) {
          std::string filename = _systems[iSys]->getSystemPath() + _systems[iSys]->getSystemName() +
                                 "_TransitionDensity_Total_" + std::to_string(i + 1);
          OutputControl::nOut << "  Printing total transition density to file: " + filename + ".cube" << std::endl;
          plotWriter->writeMatrixToGrid(filename, geom, transitionDensity.total());
          filename = _systems[iSys]->getSystemPath() + _systems[iSys]->getSystemName() +
                     "_TransitionDensity_SpinDifference_" + std::to_string(i + 1);
          OutputControl::nOut << "  Printing spin difference transition density to file: " + filename + ".cube" << std::endl;
          plotWriter->writeMatrixToGrid(filename, geom, transitionDensity.difference());
        }
        ...
        for_spin(holeDensity, particleDensity, holeDensityCorr, _fnameSuffix) {
          std::string holename = _systems[iSys]->getSystemPath() + _systems[iSys]->getSystemName() + "_HoleDensity" +
                                 _fnameSuffix_spin + std::to_string(i + 1);
          std::string particlename = _systems[iSys]->getSystemPath() + _systems[iSys]->getSystemName() +
                                     "_ParticleDensity" + _fnameSuffix_spin + std::to_string(i + 1);
          ...
          if (_systems.size() > 1) {
            OutputControl::nOut << "  Printing hole density correction (only relevant for FDEc) to file: " + holename + ".cube"
                                << std::endl;
        ...
        if (SCFMode == UNRESTRICTED) {
          std::string holename = _systems[iSys]->getSystemPath() + _systems[iSys]->getSystemName() +
                                 "_HoleDensity_Total_" + std::to_string(i + 1);
          std::string particlename = _systems[iSys]->getSystemPath() + _systems[iSys]->getSystemName() +
                                     "_ParticleDensity_Total_" + std::to_string(i + 1);
          OutputControl::nOut << "  Printing total hole density to file: " + holename + ".cube" << std::endl;
          plotWriter->writeMatrixToGrid(holename, geom, holeDensity.total());
          OutputControl::nOut << "  Printing total particle density to file: " + particlename + ".cube" << std::endl;
          plotWriter->writeMatrixToGrid(particlename, geom, particleDensity.total());
          holename = _systems[iSys]->getSystemPath() + _systems[iSys]->getSystemName() +
                     "_HoleDensity_SpinDifference_" + std::to_string(i + 1);
          particlename = _systems[iSys]->getSystemPath() + _systems[iSys]->getSystemName() +
                         "_ParticleDensity_SpinDifference_" + std::to_string(i + 1);
          OutputControl::nOut << "  Printing spin difference hole density to file: " + holename + ".cube" << std::endl;
          plotWriter->writeMatrixToGrid(holename, geom, holeDensity.difference());
          OutputControl::nOut << "  Printing spin difference particle density to file: " + particlename + ".cube\n"
                              << std::endl;
          plotWriter->writeMatrixToGrid(particlename, geom, particleDensity.difference());
        }
      }
    }
  }
