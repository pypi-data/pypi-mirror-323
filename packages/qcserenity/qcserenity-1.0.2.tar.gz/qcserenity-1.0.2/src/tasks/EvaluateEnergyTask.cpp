  ...
  if (_systemController.size() == 1) {
    ...
    if (settings.useDifferentXCFunc) {
      ...
      if (settings.customFunc.basicFunctionals.size()) {
        system->setCustomXCfunctional(settings.customFunc);
      }
    }
    ...
    system->setCustomXCfunctional(originalCustomXCfunc);
  }
  /*
   * sDFT evaluation
   */
    ...
    for (unsigned int i = 0; i < _systemController.size(); i++) {
      ...
      if (settings.useDifferentXCFunc) {
        ...
        if (settings.customFunc.basicFunctionals.size()) {
          _systemController[i]->setCustomXCfunctional(settings.customFunc);
        }
      }
    ...
    for (unsigned int i = 0; i < _systemController.size(); i++) {
      ...
      _systemController[i]->setCustomXCfunctional(originalCustomXCfunc[i]);
    }
  }
}
