...
class ScheduleBuilder:
    ...
    def __init__(  # pylint: disable=too-many-arguments
        ...
        self._variable_sample_rates: bool = len({c.sample_rate for c in self.channels.values()}) > 1
        """Whether the builder has variable sampling rates in its channels. NOTE: the drive and flux channels should
        always have uniform sampling rates, so this can happen only due to readout channels."""
        self._channel_types = {
    ...
    def validate_quantum_circuit(
        ...
        *,
        ...
        def validate_circuit_operation(op: CircuitOperation) -> None:
            """Validate a single operation in a quantum circuit."""
            # find the op type
            op_type = self.op_table.get(op.name)
            if op_type is None:
                raise ValueError(f"Unknown quantum operation '{op.name}'.")

            # find the implementation
            impl_name = op.implementation
            if impl_name is not None and impl_name not in op_type.implementations:
                raise ValueError(f"Unknown implementation '{impl_name}' for quantum operation '{op.name}'.")

            if 0 < op_type.arity != len(op.locus):
                raise ValueError(
                    f"The '{op.name}' operation acts on {op_type.arity} qubit(s), "
                    f"but {len(op.locus)} were given: {op.locus}"
                )

            if not set(op_type.params).issubset(op.args):
                raise ValueError(
                    f"The '{op.name}' operation requires the arguments {op_type.params}, "
                    f"but {tuple(op.args)} were given."
                )
    ...
    def timeboxes_to_front_padded_playlist(
        ...
        # TODO: below assumes the same amout of probe pulses in every segment of the playlist so we can just check
        # the presence of the readout channel content from the first segement. Once this restrcition is lifted
        # in the SC, we have to check every segment individually.
        if self._variable_sample_rates and schedules[0].has_content_in(self._channel_types["probe"]):
            schedules = [
