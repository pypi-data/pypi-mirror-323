...
def setup_depot_embeddings(bucket, model, computed_depot_names, computed_depot_embeddings):
    """
...
def check_formatting_rule(entity_key, document_type_code, rule):
    if document_type_code in formatting_rules.keys() and \
        entity_key in formatting_rules[document_type_code].keys() and \
        formatting_rules[document_type_code][entity_key] == rule:
        return True
    return False


async def format_label(entity_k, entity_value, embed_manager, document_type_code, llm_client):
    ...
    if isinstance(entity_value, list):
        format_tasks = await asyncio.gather(
            *[
                format_label(entity_k, sub_v, embed_manager, document_type_code, llm_client)
...
async def format_all_entities(result, embed_manager, document_type_code, llm_client):
    ...
    _, aggregated_data = await format_label(None, result, embed_manager, document_type_code, llm_client)
