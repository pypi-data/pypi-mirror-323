      ...
      <template v-slot:body-cell="props">
        <q-td :props="props">
          <div v-if="props.col.name === 'action'">
            <q-btn round flat icon="published_with_changes" @click="changPWD(props.row.id)">
...
function changPWD(e) {
  $q.dialog({
    ...
    post('core/user/changepwd/', {"id": e, "pwd": data}).then(() => {
      $q.notify({
        type: 'success',
        message: t('sucesschangepwd')
      })
    }).catch(err => {
      $q.notify({
        type: 'error',
...
function lockUser(e) {
  post('core/user/lock/', {"id": rows.value[e].id}).then(() => {
    ...
    rows.value[e].is_active = !rows.value[e].is_active
  }).catch(err => {
    $q.notify({
      type: 'error',
      message: err
