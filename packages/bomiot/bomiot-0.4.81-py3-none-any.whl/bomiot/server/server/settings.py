...
if PROJECT_NAME == 'bomiot':
    LOCALE_DIR = join(BASE_DIR, 'locale').replace('\\', '/')
else:
    LOCALE_DIR = join(join(WORKING_SPACE, PROJECT_NAME), 'locale').replace('\\', '/')
exists(LOCALE_DIR) or os.makedirs(LOCALE_DIR)

LANGUAGE_CODE = CONFIG.getint('locale', 'language_code', fallback='en')

TIME_ZONE = CONFIG.getint('locale', 'time_zone', fallback='UTC')
