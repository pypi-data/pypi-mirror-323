...
def run_req(base_folder_path: str, api_url: str, repo_id: str):
    ...
    # Dictionary to hold combined responses for each folder
    folder_responses = {}
    ...
    for folder_name in os.listdir(base_folder_path):
        ...
        if os.path.isdir(folder_path):
            ...
            if num_tokens == 0:
                print(f"Folder '{folder_name}' is empty or contains only ignored files.")
                continue
            ...
            # Generate documentation for each chunk and store responses
            responses = []
            ...
            # Combine responses for this folder
            combined_responses = "\n".join(responses)
            folder_responses[folder_name] = combined_responses

    # Combine all folder responses into a single documentation
    ...
    final_documentation = prompt(
        f"Based on the following folder-specific responses, generate a comprehensive code documentation:\n\n{all_responses_combined}"
    )

    # Send the final documentation to the API
