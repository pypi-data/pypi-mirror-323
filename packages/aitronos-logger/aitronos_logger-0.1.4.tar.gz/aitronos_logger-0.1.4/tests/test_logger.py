...
def test_info_logging_with_auto_component(logger):
    ...
    assert "status" in latest_log["progress"]
...
def test_progress_tracking(logger):
    ...
    # Test with dictionary progress
    progress_data = {"progress_percentage": 25, "remaining_time_seconds": 300}
    logger.info("Progress check 1", progress=progress_data)
    assert wait_for_log_write(expected_entries=2)
    
    data = read_log_file()
    first_log = data["entries"][-1]
    
    # Test with direct percentage
    logger.info("Progress check 2", progress=50)
    ...
    assert "status" not in first_log["progress"]
...
def test_progress_status(logger):
    """Test progress status when no progress is provided"""
    ...
    latest_log = data["entries"][-1]
    assert "status" in latest_log["progress"]
    assert latest_log["progress"]["status"] == "No progress information available"
