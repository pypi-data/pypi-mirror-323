...
class Route:
    ...
    __slots__ = ('path', 'method', 'handler', 'cors', '_pattern', 'is_static')
    ...
    def match(self, path: str) -> Optional[re.Match]:
        ...
        if self.is_static:
            return re.Match if path == self.path else None
        ...
        try:
            ...
            if event.get('version', '1.0') != '1.0':
                return LambdaResponse.create(
                    {"message": "Invalid API Gateway version. Expected REST API (v1)"},
                    status_code=400
                )
    ...
    def resolve(self, event: APIGatewayProxyV2Event, context: Any = None) -> dict[str, Any]:
        ...
        try:
            ...
            if event.get('version', '2.0') != '2.0':
                return LambdaResponse.create(
                    {"message": "Invalid API Gateway version. Expected HTTP API (v2)"},
                    status_code=400
                )
