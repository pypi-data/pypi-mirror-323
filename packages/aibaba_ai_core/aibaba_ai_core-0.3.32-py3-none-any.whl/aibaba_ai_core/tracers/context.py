...
tracing_v2_callback_var: ContextVar[Optional[AibabaAITracer]] = ContextVar(
...
def tracing_v2_enabled(
    ...
    *,
...
) -> Generator[AibabaAITracer, None, None]:
    ...
    Yields:
        AibabaAITracer: The Aibaba AI tracer.
...
def _get_trace_callbacks(
    ...
    if _tracing_v2_is_enabled():
        ...
        tracer = tracing_v2_callback_var.get() or AibabaAITracer(
        ...
        if callback_manager is None:
            ...
            if not any(
                isinstance(handler, AibabaAITracer)
                for handler in callback_manager.handlers
            ):
                callback_manager.add_handler(tracer, True)
                # If it already has a AibabaAITracer, we don't need to add another one.
