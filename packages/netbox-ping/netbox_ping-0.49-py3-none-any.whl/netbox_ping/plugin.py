...
def initialize_plugin():
    ...
    for link_data in custom_links_data:
        object_type = link_data.pop('object_type')
        ...
        if not link.object_types.filter(id=object_type.id).exists():
            link.object_types.add(object_type)
            print(f"Associated {object_type} with custom link: {link.name}")
