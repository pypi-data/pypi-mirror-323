...
***** [[elisp:(org-cycle)][| *CmndDesc:* | ]]  Conventional top level example.
        ...
        moduleName = "moduleName"  # BISOS Capabilities Bundle

        cs.examples.menuChapter(f'={moduleName} Activation Commands=')
        cmnd(f'{moduleName}_activate', comment=" # Activate")
        cmnd(f'{moduleName}_activationStatus', comment=" # Activation Status")
        cmnd(f'{moduleName}_deactivate', comment=" # De-Activate")

        return(cmndOutcome)

####+BEGIN: b:py3:cs:cmnd/classHead :cmndName "marmee_activate" :comment "" :extent "verify" :ro "cli" :parsMand "" :parsOpt "" :argsMin 0 :argsMax 0 :pyInv ""
""" #+begin_org
*  _[[elisp:(blee:menu-sel:outline:popupMenu)][±]]_ _[[elisp:(blee:menu-sel:navigation:popupMenu)][Ξ]]_ [[elisp:(outline-show-branches+toggle)][|=]] [[elisp:(bx:orgm:indirectBufOther)][|>]] *[[elisp:(blee:ppmm:org-mode-toggle)][|N]]*  CmndSvc-   [[elisp:(outline-show-subtree+toggle)][||]] <<marmee_activate>>  =verify= ro=cli   [[elisp:(org-cycle)][| ]]
#+end_org """
class marmee_activate(cs.Cmnd):
    cmndParamsMandatory = [ ]
    cmndParamsOptional = [ ]
    cmndArgsLen = {'Min': 0, 'Max': 0,}
...
: [{'networking.primary': 'eno1'}, {'os.distro.id': 'Debian'}]
        ...
        if b.subProc.WOpW(invedBy=self, log=1).bash(
                f"""marmee-binsPrep.cs  -i binsPrep_fullUpdate""",
        ).isProblematic():  return(b_io.eh.badOutcome(cmndOutcome))

        if b.subProc.WOpW(invedBy=self, log=1).bash(
                f"""marmee-assemble.cs  -i marmee_activate""",
        ).isProblematic():  return(b_io.eh.badOutcome(cmndOutcome))
