...
class DatabaseManager:
    ...
    def update_hook(
        ...
        print(f"Updating hook with ID: {hook_id}")  # Debug print

        if rowcount > 0:
            self._sync_hooks_file()
            print("Calling _sync_hooks_file()")
        return rowcount > 0
    ...
    def source_hook_mapping_exists(self, source_id, target_id):
        """Check if a source-hook mapping with the given source_id and target_id exists."""
        query = "SELECT 1 FROM source_hook_mappings WHERE source_id = ? AND target_id = ?"
        result = self._execute_query(query, (source_id, target_id),
                                     fetch_one=True)
        return bool(result)
    ...
    def source_exists(self, source_name):
        ...
        query = "SELECT 1 FROM sources WHERE source_name = ?"
        result = self._execute_query(query, (source_name,), fetch_one=True)
        return bool(result)
    ...
    def _sync_hooks_file(self) -> None:
        ...
        print("Inside _sync_hooks_file")
        hooks = self.get_all_hooks()
        print(f"hook_file: {self.flatpack_directory}")
        ...
        print(f"Writing to {hooks_file}: {data}")
