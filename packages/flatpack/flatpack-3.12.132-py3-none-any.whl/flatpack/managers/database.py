...
class DatabaseManager:
    ...
    def add_hook(
        ...
        lastrowid, _ = self._execute(
            """INSERT INTO flatpack_hooks 
               (hook_name, hook_placement, hook_script, hook_type, show_on_frontpage)
               VALUES (?, ?, ?, ?, ?)""",
    ...
    def add_source(
        ...
        lastrowid, _ = self._execute(
            "INSERT INTO flatpack_sources (source_name, source_type, source_details) VALUES (?, ?, ?)",
    ...
    def _sync_connections_file(self) -> None:
        ...
        connections_file = os.path.join(os.path.dirname(self.db_path),
                                        "connections.json")
        data = {"connections": [{
    ...
    def _sync_sources_file(self) -> None:
        """Maintain sources.json file synchronization."""
        sources = self.get_all_sources()
        sources_file = os.path.join(os.path.dirname(self.db_path),
                                    "sources.json")
        data = {"sources": [{
