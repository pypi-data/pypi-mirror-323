...
from masterpiece import TimeSeries, Mqtt, Measurement
...
class TestJuham(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.juham.database_client = MagicMock(spec=TimeSeries)
    ...
    def test_initialization(self) -> None:
        ...
        self.assertIsNotNone(self.juham.database_client)
    ...
    def test_initialize(self) -> None:
        ...
        with patch.object(self.juham, "init_database") as mock_init_db, patch.object(
            self.juham, "init_mqtt"
        ) as mock_init_mqtt:
            self.juham.initialize()
            mock_init_db.assert_called_once_with(self.juham.name)
    ...
    def test_write_with_success(self) -> None:
        ...
        self.juham.database_client.write.return_value = None
        self.juham.write(mock_measurement)
        self.juham.database_client.write.assert_called_once_with(mock_measurement)

    def test_write_with_retries(self) -> None:
        """Test the write method retries on failure."""
        mock_measurement = MagicMock(spec=Measurement)
        self.juham.database_client.write.side_effect = Exception("Write error")
        self.juham.write(mock_measurement)
        self.assertEqual(
            self.juham.database_client.write.call_count, self.juham.write_attempts
        )
