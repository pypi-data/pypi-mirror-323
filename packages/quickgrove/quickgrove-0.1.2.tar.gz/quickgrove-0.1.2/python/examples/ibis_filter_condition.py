...
def create_pruned_udf(original_udf: callable, model: PyGradientBoostedDecisionTrees, 
                     predicates: List[dict]) -> Tuple[callable, List[str]]:
...
def prune_gbdt_model(_, **kwargs):
    ...
    for name, value in parent_op.values.items():
        if name == 'prediction':
            udf_kwargs = {feat_name: parent_op.values[feat_name] 
