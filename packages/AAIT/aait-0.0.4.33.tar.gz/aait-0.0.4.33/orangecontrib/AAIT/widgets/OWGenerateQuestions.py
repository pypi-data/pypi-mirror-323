...
class OWGenerateQuestions(widget.OWWidget):
    ...
    def run(self):
        ...
        if self.data_lmstudio is not None:
            ...
            if not any(ntpath.basename(d["id"]) == ntpath.basename(self.model.lower()) for d in model_list["data"]):
                self.error("Model not found in LMStudio.")
                return
        ...
        self.thread = thread_management.Thread(self.generate_questions_on_table, data, ntpath.basename(self.model), self.embedder, llm)
