...
class AbstractOAuth2APIClient:
    ...
    def get_access_token_raw_request(self) -> str:
        logging.debug("AbstractOAuth2Api::get_access_token_raw_request")
        cache_key = self.get_access_token_cache_key()
        token = cache.get(cache_key)

        if not token:
            try:
                logging.debug("AbstractOAuth2Api::get_access_token_raw_request - Access token not found in cache, requesting new token...")
                app_config = self.get_app_config()
                ...
                logging.debug(f"AbstractOAuth2Api::get_access_token_raw_request - New access token obtained: {token}, expires in {ttl} seconds.")
            except RequestException as ex:
                logging.warning(f"RequestException: {ex}")
                if ex.response and ex.response.status_code == HTTP_401_UNAUTHORIZED:
                    cache.delete(cache_key)
