...
class HiSourcing:
    ...
    def indexing(self):
        ...
        try:
            ...
            # faiss
            index = faiss.IndexFlatL2(self.query_array.shape[1])
            index.add(self.db_array)
            
            # Ensure k doesn't exceed database size
            effective_k = min(self.k, len(self.db_df))
            self.D, self.I = index.search(self.query_array, effective_k)
    ...
    def validate(self):
        ...
        try:
            ...
            # labels in db_db
            self.label_before_sourcing = self.db_df_raw[self.db_df_raw[self.label]==1].shape[0]
            
            self.label_after_sourcing = self.sourced_db_df[self.sourced_db_df[self.label]==1].shape[0] 
