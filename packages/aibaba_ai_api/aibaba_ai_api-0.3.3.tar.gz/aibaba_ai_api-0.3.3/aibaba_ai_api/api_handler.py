...
from aibaba_ai_core._api.beta_decorator import warn_beta
from aibaba_ai_core.callbacks.base import AsyncCallbackHandler
from aibaba_ai_core.callbacks.manager import BaseCallbackManager
...
from aibaba_ai_core.tracers import RunLogPatch
...
from aibaba_ai_api._pydantic import _create_root_model
from aibaba_ai_api.callbacks import AsyncEventAggregatorCallback, CallbackEventDict
from aibaba_ai_api.lzstring import LZString
from aibaba_ai_api.playground import serve_playground
from aibaba_ai_api.schema import (
...
from aibaba_ai_api.serialization import Serializer, WellKnownLCSerializer
from aibaba_ai_api.validation import (
    BatchBaseResponse,
...
def _is_hosted() -> bool:
    return os.environ.get("HOSTED_aibaba_ai_api_ENABLED", "false").lower() == "true"
...
def _update_config_with_defaults(
    ...
    *,
    ...
    metadata = {
        ...
        "__aibaba_ai_api_version": __version__,
    }

    if endpoint:
        metadata["__aibaba_ai_api_endpoint"] = endpoint

    if _is_hosted():
        hosted_metadata = {
            "__aibaba_ai_api_hosted_git_ref": os.environ.get(
                "HOSTED_aibaba_ai_api_GIT_REF", ""
            ),
            "__aibaba_ai_api_hosted_repo_subdirectory_path": os.environ.get(
                "HOSTED_aibaba_ai_api_GIT_REPO_PATH", ""
            ),
            "__aibaba_ai_api_hosted_repo_url": os.environ.get(
                "HOSTED_aibaba_ai_api_GIT_REPO", ""
            ),
            "__aibaba_ai_api_hosted_revision_id": os.environ.get(
                "HOSTED_aibaba_ai_api_REVISION_ID", ""
            ),
            "__aibaba_ai_api_hosted_is_hosted": "true",
        }
            ...
            try:
                ...
                if raise_original_error:
                    print(
                        ...
                        "See https://api.python.langchain.com/en/latest/runnables/aibaba_ai_core.runnables.base.Runnable.html "  # noqa: E501
                    )
                    raise e
    ...
    async def create_feedback(
        ...
        if not tracing_is_enabled() or not self._enable_feedback_endpoint:
            raise HTTPException(
                ...
                + "enabled on your aibaba_ai_api server.\n"
    ...
    async def create_public_trace_link(
        ...
        if not tracing_is_enabled() or not self._enable_public_trace_link_endpoint:
            raise HTTPException(
                ...
                + "LangSmith is enabled on your aibaba_ai_api server.\n"
