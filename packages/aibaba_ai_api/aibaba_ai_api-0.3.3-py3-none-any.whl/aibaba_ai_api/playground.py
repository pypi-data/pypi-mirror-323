...
def _get_mimetype(path: str) -> str:
    ...
    Resolves issue: https://github.com/aibaba-aiaibaba_ai_api/issues/245
        ...
        with open(local_file_path, encoding="utf-8") as f:
            ...
            if mime_type in ("text/html", "text/css", "application/javascript"):
                response = PlaygroundTemplate(f.read()).substitute(
                    aibaba_ai_api_BASE_URL=base_url[1:]
                    if base_url.startswith("/")
                    else base_url,
                    aibaba_ai_api_CONFIG_SCHEMA=json.dumps(
                        runnable.config_schema(include=config_keys).model_json_schema()
                    ),
                    aibaba_ai_api_INPUT_SCHEMA=json.dumps(input_schema.model_json_schema()),
                    aibaba_ai_api_OUTPUT_SCHEMA=json.dumps(
                        output_schema.model_json_schema()
                    ),
                    aibaba_ai_api_FEEDBACK_ENABLED=json.dumps(
                        "true" if feedback_enabled else "false"
                    ),
                    aibaba_ai_api_PUBLIC_TRACE_LINK_ENABLED=json.dumps(
