...
def __getattr__(name):
    if name not in __all__:
        raise AttributeError(
                "`quantecon_wasm.discrete_rv` is deprecated and has no attribute "
                f"'{name}'."
            )

    warnings.warn(f"Please use `{name}` from the `quantecon` namespace, "
                  "the `quantecon_wasm.discrete_rv` namespace is deprecated. You "
                  "can use the following instead:\n "
                  f"`from quantecon_wasm import {name}`.",
