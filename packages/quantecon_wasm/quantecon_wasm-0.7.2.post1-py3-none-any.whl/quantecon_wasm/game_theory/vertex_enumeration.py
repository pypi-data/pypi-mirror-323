...
def _ints_arr_to_bits_single(ints_arr):
    """
...
def _ints_arr_to_bits(ints_arr):
    """
    Convert an array (1D or 2D) of integers representing the set bits into the
    corresponding integer(s).

    Parameters
    ----------
    ints_arr : ndarray(int32, ndim=1 or 2)
        Array of distinct integers from 0, ..., 63. Can be 1D or 2D.

    Returns
    -------
    np.uint64 or ndarray(uint64, ndim=1)
        If `ints_arr` is 1D, returns a single integer.
        If `ints_arr` is 2D, returns an array of integers with one integer
        for each row.

    Examples
    --------
    >>> ints_arr = np.array([0, 1, 2], dtype=np.int32)
    >>> ints_arr_to_bits(ints_arr)
    7

    >>> ints_arr2d = np.array([[0, 1, 2], [3, 0, 1]], dtype=np.int32)
    >>> ints_arr_to_bits(ints_arr2d)
    array([ 7, 11], dtype=uint64)
    """
    if ints_arr.ndim == 1:
        return _ints_arr_to_bits_single(ints_arr)
    elif ints_arr.ndim == 2:
        return np.array([_ints_arr_to_bits_single(row) for row in ints_arr], dtype=np.uint64)
    else:
        raise ValueError("Input `ints_arr` must be a 1D or 2D array.")
