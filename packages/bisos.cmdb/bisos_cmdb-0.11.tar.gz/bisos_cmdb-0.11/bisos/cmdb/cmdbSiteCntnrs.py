...
** [[elisp:(org-cycle)][| *DocStr | ] Examples of Service Access Instance Commands.
    ...
    cmnd('bpoCntnrIds', wrapper="bpoAcctManage.sh -i bpoIdsList | grep pmp_ |",
         pars=od([('bpoId', this_bpoId),]), args="py")

    cmnd('bpoCntnrIds', pars=od([('bpoId', this_bpoId),]), args="py")

    cmnd('bpoCntnrIds', wrapper="bxoGitlab.py -v 30 -i acctList | grep pmp_ |",
         pars=od([('bpoId', this_bpoId),]), args="py")

    cs.examples.menuChapter('*Prepatory Actions*')

    literal(f"""bxoGitlab.py -v 30 -i acctList | grep pmp_""")
...
####+BEGIN: b:py3:cs:cmnd/classHead :cmndName "bpoCntnrIds" :comment "" :parsMand "" :parsOpt "bpoId" :argsMin 1 :argsMax 1 :pyInv "pyStdinParams"
""" #+begin_org
*  _[[elisp:(blee:menu-sel:outline:popupMenu)][±]]_ _[[elisp:(blee:menu-sel:navigation:popupMenu)][Ξ]]_ [[elisp:(outline-show-branches+toggle)][|=]] [[elisp:(bx:orgm:indirectBufOther)][|>]] *[[elisp:(blee:ppmm:org-mode-toggle)][|N]]*  CmndSvc-   [[elisp:(outline-show-subtree+toggle)][||]] <<bpoCntnrIds>>  =verify= parsOpt=bpoId argsMin=1 argsMax=1 ro=cli pyInv=pyStdinParams   [[elisp:(org-cycle)][| ]]
#+end_org """
class bpoCntnrIds(cs.Cmnd):
    cmndParamsMandatory = [ ]
