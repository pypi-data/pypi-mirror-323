...
def _draw_matplotlib_plot(data_frame: pd.DataFrame, ci: CausalImpactAnalysis = None, generate_diagnostic_plots=True,
    ...
    2. Pointwise Effect
    3. Cumulative Effect
    4. Cumulative Intervention vs. Non-Intervention
    ...
    def process_y_formatter_units(y_formatter_unit_options, subplot_labels):
        if isinstance(y_formatter_unit_options, dict):
            return y_formatter_unit_options
        else:
            # If it's a string or list, handle accordingly
    ...
    def add_period_markers(ax, df, labels):
        ...
        if (df["time"] < pre_start).any():
            ax.axvline(pre_start, color="grey", linestyle="--", label=labels.get('pre_period_start', 'Pre-Period Start'))

        if ((df["time"] > pre_end) & (df["time"] < post_start)).any():
            ax.axvline(pre_end, color="grey", linestyle="--", label=labels.get('pre_period_end', 'Pre-Period End'))

        ax.axvline(post_start, color="grey", linestyle="--", label=labels.get('post_period_start', 'Post-Period Start'))

        if (df["time"] > post_end).any():
            ax.axvline(post_end, color="grey", linestyle="--", label=labels.get('post_period_end', 'Post-Period End'))
    ...
    dpi = 100
    # Now we have 4 subplots arranged in 2x2, adjust height accordingly
    fig_width_in = chart_width_px / dpi
    fig_height_in = (chart_height_px * 2) / dpi  # 2 rows

    x_label = plot_options.get("x_label", "Date")
    y_labels = plot_options.get("y_labels", ["Observed", "Pointwise Effect", "Cumulative Effect", "Cumulative Intervention vs. Non-Intervention"])
    ...
    y_formatter_option = plot_options.get("y_formatter", "thousands")
    y_formatter_unit_option = plot_options.get("y_formatter_unit", "Kyen")

    # Process y_formatter_unit to match subplot labels
    y_formatter_units = process_y_formatter_units(y_formatter_unit_option, y_labels)
    # Replace any missing units with default
    for label in y_labels:
        if label not in y_formatter_units:
            y_formatter_units[label] = default_unit

    # ----------------------------- Create Subplots -----------------------------
    fig, axs = plt.subplots(
        nrows=2,  # 2 rows for 4 subplots
        ncols=2,
        figsize=(fig_width_in, fig_height_in),
        sharex=True,
    ...
    for ax, y_label in zip(axs.flatten(), y_labels):
    ...
    # Set the common x-axis label on the bottom-right subplot
    axs[1, 1].set_xlabel(x_label, fontsize=axis_label_font_size, fontweight="bold")
    ...
    ax1.plot(mean_data["time"], mean_data["value"],
             label=legend_labels.get('mean', 'Predicted'), color='blue')
    ax1.plot(observed_data["time"], observed_data["value"],
             label=legend_labels.get('observed', 'Observed'), color='orange')
    ax1.fill_between(
        observed_data["time"],
        observed_data["lower"],
        observed_data["upper"],
    ...
    ax1.legend(loc="upper left", bbox_to_anchor=(1, 1), fontsize="small", frameon=False)
    ...
    ax2.plot(pointwise_data["time"], pointwise_data["value"],
             label=legend_labels.get('pointwise', 'Pointwise Effect'), color='green')
    ax2.fill_between(
        pointwise_data["time"],
    ...
    ax2.legend(loc="upper left", bbox_to_anchor=(1, 1), fontsize="small", frameon=False)
    ...
    ax3.plot(cumulative_data["time"], cumulative_data["value"],
             label=legend_labels.get('cumulative', 'Cumulative Effect'), color='red')
    ax3.fill_between(
    ...
    ax3.legend(loc="upper left", bbox_to_anchor=(1, 1), fontsize="small", frameon=False)

    # Subplot 4: Cumulative Intervention vs. Non-Intervention
    ax4 = axs[1, 1]
    if 'intervention_indicator' in data_frame.columns:
        intervention_mask = data_frame['intervention_indicator'] == True
        non_intervention_mask = data_frame['intervention_indicator'] == False

        # Plot cumulative intervention
        cumulative_intervention = data_frame[intervention_mask]['cumulative_effects_mean']
        ax4.plot(cumulative_intervention.index, cumulative_intervention.values,
                 label='Cumulative Intervention', color='blue')

        # Plot cumulative non-intervention
        cumulative_non_intervention = data_frame[non_intervention_mask]['cumulative_effects_mean']
        ax4.plot(cumulative_non_intervention.index, cumulative_non_intervention.values,
                 label='Cumulative Non-Intervention', color='green')

        # Highlight intervention start date
        intervention_dates = data_frame.loc[intervention_mask, 'post_period_start'].unique()
        if len(intervention_dates) > 0:
            intervention_date = intervention_dates[0]
            ax4.axvline(x=intervention_date, color='grey', linestyle='--', label='Intervention Start Date')
        else:
            intervention_date = None

        # Highlight non-intervention periods after intervention
        mask = (data_frame.index >= intervention_date) & (~data_frame['intervention_indicator'])
        if intervention_date and mask.any():
            false_periods = data_frame.loc[mask].copy()
            false_periods['group'] = (false_periods.index.to_series().diff() != pd.Timedelta(days=1)).cumsum()

            for _, group_dates in false_periods.groupby('group'):
                start_date = group_dates.index.min()
                end_date = group_dates.index.max()
                ax4.axvspan(start_date, end_date, color='red', alpha=0.3, label='Non-Intervention Indicator')

            # Remove duplicate legend entries
            handles, labels = ax4.get_legend_handles_labels()
            by_label = dict(zip(labels, handles))
            ax4.legend(by_label.values(), by_label.keys(), loc="upper left", fontsize="small", frameon=False)
    else:
        logger.warning("'intervention_indicator' column not found in the data_frame. Subplot 4 will be empty.")

    ax4.set_title('Cumulative Intervention vs. Non-Intervention', fontsize=14)
