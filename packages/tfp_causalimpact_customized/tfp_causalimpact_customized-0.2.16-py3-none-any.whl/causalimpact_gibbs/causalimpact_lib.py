...
class CausalImpactAnalysis:
    ...
    series : pd.DataFrame
        Time-indexed DataFrame with:
            ...
            - observed: `y_t`
            - `posterior_mean`: `ŷ_t` (posterior mean prediction)
            - `posterior_lower`, `posterior_upper`: Credible interval bounds for `ŷ_t`.
            - `point_effects_mean`, `point_effects_lower`, `point_effects_upper`: Mean and CI for point effects `y_t - ŷ_t`.
            - `cumulative_effects_mean`, `cumulative_effects_lower`, `cumulative_effects_upper`: Mean and CI for cumulative effects.
            - `intervention_indicator`: Boolean indicating post-intervention period.
    ...
    convergence_diagnostics : Dict[str, Any]
        Diagnostics such as R-hat and Effective Sample Size (ESS).
    """
    series: pd.DataFrame
    summary: pd.DataFrame
    posterior_samples: CausalImpactPosteriorSamples
...
def _compute_impact_estimates(posterior_trajectory_summary: pd.DataFrame,
    ...
    - point_effects_mean: `E[y_t - ŷ_t]`
    - cumulative_effects_mean: `∑(y_τ - ŷ_τ)`

    Parameters
    ----------
    posterior_trajectory_summary : pd.DataFrame
        Summary with posterior_mean and intervals of `ŷ_t`.
    ...
    -------
    pd.DataFrame
        DataFrame with observed, predicted, effects, intervals, and intervention_indicator.
    """
    # Calculate point effects
    ...
    # Check if 'intervention_indicator' exists
    if 'intervention_indicator' in point_effects_mean.columns:
        intervention_indicator = point_effects_mean["intervention_indicator"]
    else:
        # Create 'intervention_indicator' based on post_intervention_period
        pre_start, pre_end = ci_data.pre_intervention_period
        post_start, post_end = ci_data.post_intervention_period

        # Initialize as False
        intervention_indicator = pd.Series(False, index=point_effects_mean.index)

        # Set to True for post-intervention period
        intervention_indicator.loc[
            (intervention_indicator.index >= post_start) &
            (intervention_indicator.index <= post_end)
            ] = True

        # Assign to DataFrame
        point_effects_mean["intervention_indicator"] = intervention_indicator

        # Issue a warning
        warnings.warn(
            "'intervention_indicator' column not found in the data. "
            "It has been automatically created based on the post-intervention period.",
            FutureWarning
        )

    # Create a copy for cumulative effects
    cum_effects_mean_base = point_effects_mean.copy()

    # **Fix 1:** Modify only 'point_effects_mean' where intervention_indicator is False
    cum_effects_mean_base.loc[~intervention_indicator, 'point_effects_mean'] = 0

    # Perform cumulative sum on 'point_effects_mean'
    cum_effects_mean = cum_effects_mean_base['point_effects_mean'].cumsum().to_frame(name="cumulative_effects_mean")

    # Calculate quantiles for pointwise and cumulative effects
    ...
    # Set non-relevant periods to NaN
    impact_estimates.loc[
        ...
        impact_estimates.columns.difference(
            ["observed", "posterior_mean", "posterior_lower", "posterior_upper", "intervention_indicator"]
        )] = np.nan

    # Handle any remaining NaNs in 'observed'
    impact_estimates.loc[
    ...
    # Ensure 'intervention_indicator' aligns with the index
    impact_estimates["intervention_indicator"] = point_effects_mean["intervention_indicator"]

    # Add period boundaries for plotting
