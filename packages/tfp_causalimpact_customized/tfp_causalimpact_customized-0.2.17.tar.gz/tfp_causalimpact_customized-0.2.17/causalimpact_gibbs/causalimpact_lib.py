...
def _compute_impact_estimates(posterior_trajectory_summary: pd.DataFrame,
    ...
    if not 0 < quantiles[0] < quantiles[1] < 1:
        raise ValueError("`quantiles` must be a tuple with values between 0 and 1.")

    # Extract the intervention indicator from the original data
    if 'intervention_indicator' in ci_data.data.columns:
        intervention_indicator = ci_data.data['intervention_indicator']
    else:
        # If not present, set intervention_indicator to True for the post-period
        intervention_indicator = pd.Series(
            False, index=ci_data.data.index, name='intervention_indicator'
        )
        intervention_indicator.loc[ci_data.post_intervention_period[0]:] = True

    # Add the intervention indicator to the output
    ...
    # Handle cumulative effects
    cum_effects_mean_base = point_effects_mean.copy()
    cum_effects_mean_base.loc[~intervention_indicator] = 0  # Only accumulate where intervention_indicator is True
    cum_effects_mean = cum_effects_mean_base.cumsum()
    cum_effects_mean.columns = ["cumulative_effects_mean"]

    # Compute point and cumulative effects quantiles
    ...
    # Construct the final impact estimates DataFrame
    ...
    # Handle NaN values outside the pre/post periods
    ...
    # Reindex to match the full data index
    ...
    impact_estimates["intervention_indicator"] = intervention_indicator
