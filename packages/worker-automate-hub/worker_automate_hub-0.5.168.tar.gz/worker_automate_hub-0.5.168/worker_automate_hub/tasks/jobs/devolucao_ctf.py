    ...
    try:
        ...
        if pesquisar_venda_devolucao["IsOpened"] == True:
            ...
            try:
                ...
                get_pop_information = await ocr_title(numero_cupom_fiscal, "Information", "Information")
                console.print(f'retorno:{get_pop_information.sucesso}')
                console.print(f'retorno:{get_pop_information}')
                if get_pop_information.sucesso == True:
                    msg_retorno = get_pop_information.retorno
                    console.print(msg_retorno)
                    pattern = r"sequ[êe]ncia:\s*(\d+)"
                    match = re.search(pattern, msg_retorno)

                    if match:
                        numero_nota_fiscal = match.group(1)
                        console.print(f'Numero da nota fiscal {numero_nota_fiscal}')
                    ...
                    for item in itens_nota:
                        ...
                        if descricao not in item_devolvido:
                            ...
                            for item in itens_to_select:
                                if nop_selected_value in item and (('c/' in item or 'c /' in item) and ('s/' in item or 's /' in item)):
