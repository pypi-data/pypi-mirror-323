...
def screenshot(save_dir):
    ...
    hdc_mem_dc = gdi32.CreateCompatibleDC(hdc_screen)
    # 创建一个与屏幕兼容的位图
    h_bitmap = gdi32.CreateCompatibleBitmap(hdc_screen, width, height)
    # 将位图选入内存设备上下文
    gdi32.SelectObject(hdc_mem_dc, h_bitmap)

    # 定义 BITMAPINFO 结构体
    bmi = BITMAPINFO()
    bmi.bmiHeader.biSize = ctypes.sizeof(BITMAPINFOHEADER)
    bmi.bmiHeader.biWidth = width
    bmi.bmiHeader.biHeight = -height  # 负数表示从顶部开始
    ...
    # 复制屏幕内容到位图
    gdi32.BitBlt(hdc_mem_dc, 0, 0, width, height, hdc_screen, 0, 0, 0x00CC0020)

    # 分配内存来存储位图数据
    buffer = ctypes.create_string_buffer(width * height * 4)
    # 获取位图数据
    gdi32.GetDIBits(hdc_mem_dc, h_bitmap, 0, height, buffer, ctypes.byref(bmi), 0)

    # 保存为 BMP 文件
    os.chdir(save_dir)
    bmp_filename = "screenshot.bmp"
    with open(bmp_filename, 'wb') as f:
        # 写入 BMP 文件头
        f.write(struct.pack('<2sIHHI', b'BM', 14 + 40 + width * height * 4, 0, 0, 14 + 40))
        # 写入 BITMAPINFOHEADER
    ...
    # 将 BMP 转换为 JPEG
    jpg_filename = "screenshot.jpg"
    try:
        # 调用画图工具进行转换
        subprocess.run(['mspaint.exe', '/s', '/i', bmp_filename, jpg_filename], check=True)
        print(f"已将截屏保存为 {jpg_filename}")
    except subprocess.CalledProcessError as e:
        print(f"转换过程中出现错误: {e}")
...
print('Hello! hyypns 0.1.5 ('+version,'',timecur(),getcwd(),
      desktop()+')')
...
class Screen:
    ...
    global move, MOUSEEVENTF_RIGHTUP, key_mapping, press
    ...
    def press_keys(key_names):
        """
        根据键名列表模拟键盘组合键操作
        :param key_names: 键名列表，如 press_keys(['win','r']) 就能打开‘运行’
        """
        key_codes = []
...
if __name__ == '__main__':
    ...
    input('hyypns module')
    Screen.press_keys(['win','r'])
    Screen.type_text('woshi')