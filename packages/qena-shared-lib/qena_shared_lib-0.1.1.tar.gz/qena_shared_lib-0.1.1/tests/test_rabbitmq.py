...
async def test_rabbitmq_connection_manager_wrong_listeners():
    ...
    with raises(TypeError):
        RabbitMqManager([WrongListener()])  # type: ignore

    with raises(TypeError):
        RabbitMqManager([WrongListener])  # type: ignore

    with raises(TypeError):
        RabbitMqManager([1])  # type: ignore
...
async def test_rabbitmq_connection_management_wrong_inner_listener(logstash):
    ...
    with raises(AttributeError):
        RabbitMqManager(listeners=[NoInnerListener], logstash=logstash)

    with raises(TypeError):
        RabbitMqManager(listeners=[WrongInnerListener], logstash=logstash)


@mark.asyncio
async def test_rabbitmq_exception_handler_exception(logstash):
    rabbit_mq_manager = RabbitMqManager(listeners=[], logstash=logstash)
...
async def test_rabbitmq_class_based_listener(rabbitmq, logstash):
    ...
    rabbitmq_connector = RabbitMqManager(
        listeners=[TestConsumer, TestRpcWorker],
        parameters=rabbitmq.get_connection_params(),
        logstash=logstash,
    )
