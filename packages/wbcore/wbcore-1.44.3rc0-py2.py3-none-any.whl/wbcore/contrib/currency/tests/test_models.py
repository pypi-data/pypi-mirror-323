...
@pytest.mark.currency_model_tests
@pytest.mark.with_db
...
class TestSpecificModels:
    ...
    def test_convert(
        self, exact_lookup, mocker: MockerFixture, base_currency, other_currency, base_fx_rate, other_fx_rate
    ):
        # Arrange
        today = timezone.now().date()
        if exact_lookup:
            mocker.patch(
                "wbcore.contrib.currency.models.CurrencyFXRates.objects.get", side_effect=[base_fx_rate, other_fx_rate]
            )
        else:
            mock_filter = mocker.patch("wbcore.contrib.currency.models.CurrencyFXRates.objects.filter")
            mock_filter.return_value.order_by.return_value.first.side_effect = [base_fx_rate, other_fx_rate]
