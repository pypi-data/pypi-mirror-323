...
def get_file_paths():
    ...
    """
    ignore = {"venv", ".git", "DS_Store", "__pycache__", ".idea", ".vscode", "node_modules"}
    files = [
        os.path.join(dirpath, f)
        for (dirpath, dirnames, filenames) in os.walk(".")
        if not any(i in dirpath for i in ignore)
        for f in filenames
        if not any(f.endswith(i) for i in ignore)
    ]
        ...
        with tarfile.open(tarball_path, "w:gz") as tar:
            for item in os.listdir(cwd):
                if item == tarball_name:
                    ...
                    continue  # Don't add virtual environment
