        ...
        while True:
            with contextlib.suppress(ValueError):
                if hasattr(sys, "_getframemodulename"):
                    name = sys._getframemodulename(depth) or default
                else:
                    name = sys._getframe(depth).f_globals.get("__name__", default)
            if name != "__main__":
                return name.split(".")[0]
            depth += 1
            if depth > 100:
...
def _link_fp_markup(line: str | Any, fn: str | Path, lineno: int | str,render=True) -> Text | str:
...
def link_fp(line, fn, lineno,markup=True) -> str | Text:
    """Create a clickable file link with line number."""
    if not Path(fn).exists():
        fn = Path(caller("package")) / str(fn)
        if not fn.exists():
            import site
            pref = site.getsitepackages()[0]
            print(f"pref: {pref}")

            p = pref + "/" + str(fn.parent.name) + "/" + str(fn.name)
            print(f"p: {p}")
            exit()
            if not fn.exists():
    ...
    if markup:
        return _link_fp_markup(line, fn, lineno)
...
class Traceback:
    ...
    def extract(
        ...
        *,
        ...
        #         is_cause=is_cause,
        #         last_instruction=exc_value.last_instruction,
        #     )
