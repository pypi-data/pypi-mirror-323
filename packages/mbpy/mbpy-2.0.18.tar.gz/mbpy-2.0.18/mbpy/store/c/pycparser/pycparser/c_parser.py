...
class CParser(PLYParser):
    ...
    def _add_typedef_name(self, name, coord):
        ...
        if not self._scope_stack[-1].get(name, True):
            self._parse_error(
                f"Typedef {name!r} previously declared as non-typedef "
                "in this scope", coord)
    ...
    def _add_identifier(self, name, coord):
        ...
        if self._scope_stack[-1].get(name, False):
            self._parse_error(
                f"Non-typedef {name!r} previously declared as typedef "
                "in this scope", coord)
    ...
    def p_specifier_qualifier_list_4(self, p) -> None:
        ...
        p[0] = {'qual': p[1], 'alignment': [], 'storage': [], 'type': [p[2]], 'function': []}

    def p_specifier_qualifier_list_5(self, p) -> None:
        """specifier_qualifier_list  : alignment_specifier."""
        p[0] = {'qual': [], 'alignment': [p[1]], 'storage': [], 'type': [], 'function': []}
    ...
    def p_struct_declaration_1(self, p) -> None:
        ...
        if p[2] is not None:
            ...
            #
            node = spec['type'][0]
            decl_type = node if isinstance(node, c_ast.Node) else c_ast.IdentifierType(node)
