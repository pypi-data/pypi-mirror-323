...
@lru_cache(None)
def get_executor(kind: 'Literal["process", "thread", "as_completed"]') -> "ThreadPoolExecutor | ProcessPoolExecutor | Callable[...,Iterable[Coroutine[Any, Any, Any]]]":
...
def run_async(coro):
    ...
    try:
        ...
        # loop.add_signal_handler(signal.SIGKILL, _sig_kill)
    finally:
        loop.add_signal_handler(signal.SIGINT, _do_waitpid_all)
    if loop.is_running():
