...
async def _build_command(path: 'Path' = ".", env=None) -> None:
    ...
    if TYPE_CHECKING:
        ...
        from mbpy.helpers._setup_py import CompilationResult
        from mbpy.helpers._display import getconsole,SPINNER,prompt_ask
...
async def check_install_prompt(
    ...
    if TYPE_CHECKING:
        import os
        from mbpy.helpers._display import prompt_ask, confirm
        from mbpy.cmd import arun
    else:
        os = smart_import("os")
        prompt_ask = smart_import("mbpy.helpers._display.prompt_ask")
